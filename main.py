...
def user_main_keyboard(is_admin_user=False):
    """Главная клавиатура пользователя в ЛС (без кнопки Контрабанда)."""
    buttons = [
...
def completed_giveaways_keyboard(giveaways, page, total_pages):
    """Инлайн-клавиатура для архива розыгрышей."""
    kb = []
    for gw in giveaways:
...
async def process_roulette_bet(message: types.Message, state: FSMContext):
    ...
    await message.answer(phrase)
    await state.finish()
# ==================== ЧАСТЬ 5: ГРУППОВЫЕ ХЕНДЛЕРЫ, БОССЫ, ПОДГОН, КОНТРАБАНДА В ЧАТАХ ====================
...
async def finish_giveaway_winners(message: types.Message, state: FSMContext):
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            # Сохраняем список всех победителей в поле description (или создадим новое поле, но пока так)
            winner_names = []
            for wid in winners:
            ...
            # Обновим описание, чтобы сохранить список победителей
...
async def manual_spawn_boss_image(message: types.Message, state: FSMContext):
    ...
    await state.finish()
# ==================== ЧАСТЬ 7: АДМИНИСТРАТИВНЫЕ ХЕНДЛЕРЫ (ПРОДОЛЖЕНИЕ) ====================
...
async def reject_chat_callback(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        await safe_send_message(request['requested_by'], f"❌ Запрос на активацию чата «{request['title']}» отклонён.")
    await callback.answer()
# ==================== ЧАСТЬ 8: ФОНОВЫЕ ЗАДАЧИ, ВЕБ-СЕРВЕР, ТОЧКА ВХОДА И НЕДОСТАЮЩИЕ ФУНКЦИИ ====================

# ==================== НЕДОСТАЮЩИЕ ФУНКЦИИ ДЛЯ РАБОТЫ С ПОЛЬЗОВАТЕЛЯМИ ====================
...
async def on_startup(dp):
    await before_start()
    await create_db_pool()  # теперь с повторными попытками
    await init_db()
    await init_business_types()

    # Дополнительная проверка наличия всех колонок (на случай, если ранее не добавились)
    async with db_pool.acquire() as conn:
        # Проверяем и добавляем принудительно
