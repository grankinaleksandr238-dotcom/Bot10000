...
async def init_db():
    async with db_pool.acquire() as conn:
        ...
        # ---- Таблица типов бизнесов (с новыми полями) ----
        await conn.execute('''
        ...
        # ---- Таблица последних ставок (для кнопки повтора) ----
        await conn.execute('''
    ...
    await init_business_types()  # создаём 6 бизнесов
...
async def init_business_types():
    """Инициализация 6 типов бизнесов с описанием, ценой в BTC и доходом."""
    async with db_pool.acquire() as conn:
        count = await conn.fetchval("SELECT COUNT(*) FROM business_types")
        if count == 0:
            businesses = [
                # (name, emoji, price_btc, income_cents_per_hour, description, max_level)
...
async def ensure_user_exists(user_id: int, username: str = None, first_name: str = None):
    """Гарантирует, что пользователь есть в БД. Если нет – создаёт с балансом 0."""
    async with db_pool.acquire() as conn:
...
async def check_global_cooldown(user_id: int, command: str) -> Tuple[bool, int]:
    """Проверяет глобальный кулдаун на команду. Возвращает (можно_ли, секунд_осталось)."""
...
async def get_business_price(business_type: dict, level: int) -> float:
    """Стоимость улучшения бизнеса до указанного уровня."""
    if level == 1:
        return float(business_type['base_price_btc'])
    else:
...
async def collect_business_income(user_id: int, business_id: int) -> Optional[float]:
    async with db_pool.acquire() as conn:
...
async def upgrade_business(user_id: int, business_id: int) -> Tuple[bool, str]:
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            cost = await get_business_price({'base_price_btc': biz['base_price_btc']}, current_level + 1)
...
async def get_order_book() -> Dict[str, List[Dict]]:
    """Возвращает структуру стакана: покупка и продажа, сгруппированные по цене."""
    async with db_pool.acquire() as conn:
        ...
        """)
        return {
            'bids': [dict(r) for r in buy_orders],
            'asks': [dict(r) for r in sell_orders]
        }
...
async def match_orders(conn):
    """Ищет пересекающиеся заявки и исполняет их."""
    while True:
        # Находим самую высокую цену покупки и самую низкую цену продажи
        ...
        if not buy or not sell or buy['price'] < sell['price']:
            break  # нет пересечения

        # Определяем объём сделки
        trade_amount = min(buy['amount'], sell['amount'])
        trade_price = sell['price']  # цена продавца (или можно среднюю)
        ...
        new_buy_locked = buy['total_locked'] - total_cost
        new_sell_locked = sell['total_locked'] - trade_amount
        ...
        # Продолжаем цикл, пока есть пересечения
...
def order_book_keyboard(book: Dict[str, List[Dict]]):
    """Инлайн-клавиатура для отображения стакана с кнопками действий."""
    kb = []
    if book['asks']:
        kb.append([InlineKeyboardButton(text="📉 Продажа (ASK) - лучшие цены", callback_data="noop")])
        for ask in book['asks'][:5]:  # показываем первые 5
...
# ----- Клавиатуры для повтора ставок (добавляются после игр) -----
def repeat_bet_keyboard(game: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🔁 Повторить", callback_data=f"repeat_{game}")]
    ])

# ----- Административные клавиатуры (переработаны для компактности) -----
def admin_main_keyboard(permissions: List[str]):
    """Главное меню админ-панели сгруппировано по разделам."""
    buttons = []
    row1 = []
...
def admin_business_keyboard():
    """Клавиатура для управления бизнесами."""
    return ReplyKeyboardMarkup(keyboard=[
...
async def save_last_bet(user_id: int, game: str, amount: float, bet_data: dict = None):
    """Сохраняет последнюю ставку пользователя для кнопки повтора."""
    async with db_pool.acquire() as conn:
        await conn.execute("""
...
async def guess_number(message: types.Message, state: FSMContext):
    ...
    if win:
        ...
        await save_last_bet(user_id, 'guess', amount)
...
async def repeat_bet_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    # Запускаем соответствующую игру (функции ниже)
    if game == 'casino':
    ...
    await callback.answer()
    # Не удаляем сообщение, чтобы сохранить историю

# Вспомогательные функции для повтора (вынос логики)
...
async def process_guess_repeat(user_id: int, amount: float, number: int, message: types.Message):
    ...
    await save_last_bet(user_id, 'guess', amount, {'number': number} if win else None)
    await message.answer(phrase, reply_markup=repeat_bet_keyboard('guess'))
...
async def process_roulette_repeat(user_id: int, amount: float, bet_type: str, number: int, message: types.Message):
    ...
    bet_data = {'bet_type': bet_type, 'number': number} if win else None
    await save_last_bet(user_id, 'roulette', amount, bet_data)
    ...
    await save_last_bet(user_id, 'roulette', amount, {'bet_type': bet_type, 'number': number} if win else None)
    await message.answer(phrase, reply_markup=repeat_bet_keyboard('roulette'))
...
async def auction_view(callback: types.CallbackQuery):
    ...
    if bids:
        for bid in bids:
            ...
            text += f"• {user or 'Неизвестно'}: {float(bid['bid_amount']):.2f} баксов ({bid['bid_time']})\n"
    else:
...
async def business_view(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        await update_business_income(user_id, conn)
        biz = await conn.fetchrow(
            "SELECT ub.*, bt.name, bt.emoji, bt.base_price_btc, bt.base_income_cents, bt.max_level FROM user_businesses ub "
            "JOIN business_types bt ON ub.business_type_id = bt.id WHERE ub.id=$1 AND ub.user_id=$2",
            biz_id, user_id
        )
        if not biz:
            await callback.answer("Бизнес не найден", show_alert=True)
...
async def business_collect(callback: types.CallbackQuery):
    ...
    if coins is None:
        ...
        await callback.answer(f"✅ Собрано {coins:.2f} баксов!", show_alert=True)
        ...
        async with conn.transaction():
            for order in orders:
                ...
                if not current or current['amount'] < take - 0.0001:
                    continue  # ордер изменился, пропускаем (можно усложнить логику)
                # Обновляем балансы
...
# ==================== КОНЕЦ ЧАСТИ 5 ====================
# ==================== ЧАСТЬ 6: МУЛЬТИПЛЕЕР (ИГРА 21) – ПОЛНОСТЬЮ ПЕРЕРАБОТАННЫЙ ====================
...
async def finish_game(game_id: str):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if winner_id:
                await safe_send_message(winner_id, f"🎉 Ты выиграл в игре 21! Твой выигрыш: {pot:.2f} баксов.")
            else:
                for p in players:
...
async def cmd_help_chat(message: types.Message):
    ...
    text = (
        ...
        "/mlb_fight – атаковать банду (раз в 30 мин)\n"
        "/mlb_gym – улучшить характеристики за авторитет\n"
        "/mlb_status – твой статус\n"
        "/mlb_top – топ чата\n"
        "/mlb_boss_status – текущее HP босса\n"
        "/mlb_smuggle_chat – отправиться в контрабандный рейс (результат будет в чате)\n"
...
async def remove_balance_amount(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await safe_send_message(uid, f"💸 У тебя списано {amount:.2f} баксов администратором.")
    except Exception as e:
...
async def create_task_target(message: types.Message, state: FSMContext):
    ...
    await message.answer("Введи награду (баксы, можно дробно):")
    await CreateTask.next()
...
async def create_task_reward_coins(message: types.Message, state: FSMContext):
    ...
    await message.answer("Введи награду (репутация, целое число):")
    await CreateTask.next()
...
async def delete_ad_start(message: types.Message):
    ...
    await CancelAuction.auction_id.set()  # переиспользуем состояние
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if order['type'] == 'sell':
                ...
                await update_user_balance(order['user_id'], float(order['total_locked']), conn=conn)
            await conn.execute("UPDATE bitcoin_orders SET status='cancelled' WHERE id=$1", order_id)
...
async def settings_exp(message: types.Message):
    ...
    params = [
        ...
        ("exp_per_fight", "⚔️ Опыт за бой в чате"),
...
async def show_settings_category(message: types.Message, params: List[Tuple[str, str]], category: str):
    ...
    for key, desc in params:
        ...
        try:
            if '.' in value:
                value = f"{float(value):.2f}".rstrip('0').rstrip('.') if '.' in value else value
        except:
            pass
        text += f"{desc}: {value}\n"
...
async def edit_setting_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"✏️ Введи новое значение для параметра `{key}` (текущее: {current}):")
            ...
            async with db_pool.acquire() as conn:
                ...
                for auction in expired:
                    ...
                    if winner_bid:
                        ...
                        await safe_send_message(
                            winner_id,
                            f"🎉 Поздравляем! Вы выиграли аукцион «{auction['item_name']}» с ценой {float(final_price):.2f} баксов. Админ скоро свяжется."
                        )
                        await safe_send_message(
                            auction['created_by'],
                            f"🏁 Аукцион «{auction['item_name']}» завершён. Победитель: {winner_id}, цена: {float(final_price):.2f}."
                        )
                    else:
                        await conn.execute(
...
async def on_startup(dp):
    await bot.set_my_commands([
        ...
        types.BotCommand("mlb_fight", "Бой в чате"),
        types.BotCommand("mlb_gym", "Качалка"),
        types.BotCommand("mlb_status", "Статус в чате"),
        types.BotCommand("mlb_top", "Топ чата"),
        types.BotCommand("mlb_boss_status", "Статус босса"),
        types.BotCommand("mlb_smuggle_chat", "Контрабанда в чате"),
        types.BotCommand("mlb_help", "Помощь по чату"),
    ])
