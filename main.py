...
async def safe_send_message(user_id: int, text: str, **kwargs):
    # Экранируем только если явно указан HTML и нужно экранировать весь текст
    # Лучше экранировать отдельные переменные при формировании строки
    try:
...
# ==================== РАБОТА С НАСТРОЙКАМИ (ИСПРАВЛЕННАЯ) ====================
async def get_setting(key: str) -> str:
    ...
    # Безопасное получение: если нет в кэше, берем из DEFAULT и сохраняем в БД
    value = settings_cache.get(key)
    if value is None:
        value = DEFAULT_SETTINGS.get(key, "")
        if value:
            # Асинхронно добавим отсутствующую настройку в БД, чтобы она появилась в кэше
            async with db_pool.acquire() as conn:
                await conn.execute(
                    "INSERT INTO settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO NOTHING",
                    key, value
...
async def set_setting(key: str, value: str):
    ...
    settings_cache[key] = value
    # сбрасываем время кэша, чтобы принудительно обновить при следующем get
...
async def upgrade_business(user_id: int, business_id: int) -> Tuple[bool, str]:
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            cost = await get_business_price({'base_price_btc': biz['base_price_btc']}, biz['level'] + 1)
...
# ==================== ФУНКЦИИ ДЛЯ КОНТРАБАНДЫ (ИСПРАВЛЕННЫЕ) ====================
async def check_smuggle_cooldown(user_id: int) -> Tuple[bool, int]:
    async with db_pool.acquire() as conn:
        ...
        if row and row['cooldown_until']:
            # Если строка, конвертируем, если уже datetime - оставляем
...
# ==================== ФУНКЦИИ ДЛЯ ЭКСПОРТА (ИСПРАВЛЕННЫЕ) ====================
...
async def export_table_to_csv(table: str) -> Optional[bytes]:
    ...
    async with db_pool.acquire() as conn:
        try:
            # Дополнительная проверка существования таблицы
            ...
            if not exists:
                return None
            # Безопасное формирование запроса: имя таблицы проверено, но всё равно используем экранирование
...
# ----- СОСТОЯНИЕ ДЛЯ УДАЛЕНИЯ РЕКЛАМЫ (НОВОЕ) -----
...
async def save_last_bet(user_id: int, game: str, amount: float, bet_data: dict = None):
    """Сохраняет последнюю ставку пользователя для кнопки повтора. Всегда сохраняем bet_data, даже если None."""
    async with db_pool.acquire() as conn:
        await conn.execute("""
...
async def guess_number(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            ...
            bet_data = {'number': guess}  # сохраняем число даже при проигрыше для повтора
        await add_exp(user_id, exp, conn=conn)
...
async def process_roulette_bet(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            ...
            bet_data = {'bet_type': bet_type, 'number': bet_number}  # сохраняем параметры даже при проигрыше
        await add_exp(user_id, exp, conn=conn)
...
# ----- Обработчик повтора ставки (ИСПРАВЛЕННЫЙ) -----
        ...
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                if success and victim_balance > 0:
                    # Исправленная логика: нельзя украсть меньше минимальной суммы
                    if victim_balance < min_amount:
                        steal_amount = 0  # кража невозможна
                    else:
                    ...
                    if steal_amount > 0:
                        ...
                        # Недостаточно денег у жертвы для минимальной кражи
                        await conn.execute("UPDATE users SET theft_attempts = theft_attempts + 1, theft_failed = theft_failed + 1 WHERE user_id=$1", robber_id)
                        exp_fail = await get_setting_int("exp_per_theft_fail")
                        await add_exp(robber_id, exp_fail, conn=conn)
...
async def tasks_unified_handler(message: types.Message):
    ...
    # Если пользователь админ и имеет права на управление заданиями, показываем админ-меню
...
async def auction_unified_handler(message: types.Message):
    ...
    # Если админ и имеет права на управление аукционами, показываем админ-меню
...
async def auction_bid_amount(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        # Проверка, что пользователь не является текущим лидером
...
async def activate_chat_command(message: types.Message):
    ...
    # Проверяем, не подтвержден ли уже чат
...
async def cmd_boss_attack(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        # Проверяем, не атаковал ли пользователь уже этого босса
...
# ==================== ПОДГОН (GIFT) В ЧАТЕ ====================
# Эта кнопка должна появляться в подтвержденных чатах автоматически
# Для этого бот должен отправлять сообщение с кнопкой при активации чата
# Но также можно добавить обработчик текста "🎁 Подгон"
...
# ==================== ФОНОВАЯ ЗАДАЧА: СПАВН БОССОВ (ОПТИМИЗИРОВАННАЯ) ====================
async def boss_spawn_scheduler():
    while True:
        try:
            ...
            # Оптимизированный выбор случайного чата через SQL
            async with db_pool.acquire() as conn:
            ...
            async with db_pool.acquire() as conn3:
                image_file_id = await conn3.fetchval(
