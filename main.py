...
async def set_smuggle_cooldown(user_id: int, penalty: int = 0):
    ...
    # Устанавливаем кулдаун через глобальную таблицу
    async with db_pool.acquire() as conn:
        await conn.execute('''
            INSERT INTO global_cooldowns (user_id, command, last_used)
            VALUES ($1, $2, $3)
        ...
        for biz in businesses:
            if biz['last_collection']:
                ...
                if hours_passed > 0:
                    ...
                    max_storage = await conn.fetchval(
                        "SELECT max_storage FROM business_types WHERE id=$1",
                        biz['business_type_id']
                    )
                    if new_accum > max_storage:
                        new_accum = max_storage
                    await conn.execute(
...
# - Остальные хендлеры (аукцион, реклама, статистика, блокировки, рассылка, настройки, управление админами) обновлены с учётом новых реалий (datetime и т.д.)
...
# ==================== В ЭТОЙ ВЕРСИИ ДОБАВЛЕНО ====================
# - Инициализация типов бизнесов (init_business_types)
# - Запуск фоновой задачи smuggle_check_loop
# - Очистка старых записей из smuggle_runs в perform_cleanup
# - Обновлён on_startup с вызовом всех новых функций
...
async def init_business_types():
    ...
    async with db_pool.acquire() as conn:
        ...
        if count == 0:
            businesses = [
                # name, description, cost_smuggle, income_per_hour (в центах), max_storage (в центах), required_authority
...
async def perform_cleanup(manual=False):
    ...
    days_smuggle = int(await get_setting("cleanup_days_smuggle"))  # новая настройка
            ...
            async with db_pool.acquire() as conn:
                ...
                for run in runs:
                    ...
                    if rand <= success_chance:
                        ...
                        result = "caught"
                        # Попался – добавляем штрафной кулдаун
                        penalty = int(await get_setting("smuggle_fail_penalty_minutes"))
                        # Устанавливаем кулдаун со штрафом
                        await set_global_cooldown(user_id, "smuggle")
                        # Добавляем дополнительное время (костыль: обновим запись в global_cooldowns, добавив penalty минут)
                        # Для этого нужно получить текущее время и прибавить penalty минут
                        async with db_pool.acquire() as conn2:
                            await conn2.execute(
...
async def on_startup(dp):
    ...
    await init_db()
    await add_auto_delete_field()          # из части 6 (если ещё не)
    await add_missing_user_columns()       # из части 6
    await add_smuggle_columns()            # из части 7
    await create_smuggle_tables()          # из части 7
    await init_business_types()            # новая функция
