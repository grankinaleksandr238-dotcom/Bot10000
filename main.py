...
async def safe_send_message(user_id: int, text: str, **kwargs):
    # Экранируем только если явно указан HTML
...
async def universal_back_handler(message: types.Message, state: FSMContext):
    ...
    elif current_state.startswith('CreateAd') or current_state.startswith('EditAd'):
        await state.finish()
...
# ==================== БИЗНЕСЫ (исправленная версия) ====================

# Вспомогательная функция для периодического обновления дохода
...
async def my_businesses(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        await update_business_income(user_id, conn)  # эта функция у вас уже есть
...
# ==================== КОМАНДА /mlb_help В ЧАТЕ (ИСПРАВЛЕННАЯ, С ПОЛНЫМ СПИСКОМ) ====================
...
async def cmd_help_chat(message: types.Message):
    ...
    text = (
        ...
        "🎁 <b>Подгон (для админов чата):</b>\n"
        "Кнопка \"🎁 Подгон\" – Отправить подарок случайному админу\n\n"
        "ℹ️ <b>Другое:</b>\n"
...
# ==================== КОНЕЦ ЧАСТИ 7 ====================
# ==================== ЧАСТЬ 8: АДМИНИСТРАТИВНАЯ ПАНЕЛЬ (ИСПРАВЛЕННАЯ) ====================
...
# Здесь должны быть обработчики для каждой категории настроек, но они уже были представлены ранее.
...
# ==================== ФОНОВАЯ ЗАДАЧА: ОБРАБОТКА КОНТРАБАНДНЫХ РЕЙСОВ (ИСПРАВЛЕННАЯ) ====================
...
if __name__ == '__main__':
    ...
    loop.create_task(update_all_businesses_income())  # новая задача
