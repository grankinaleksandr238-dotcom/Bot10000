# ==================== ПОЛНЫЙ РАБОЧИЙ БОТ (ИТОГОВАЯ ВЕРСИЯ) ====================
# Единый файл. Все функции, хендлеры, состояния, клавиатуры и фоновые задачи.
# ==================== В ЭТОЙ ВЕРСИИ РЕАЛИЗОВАНО ВСЁ ====================
# - Полная система контрабанды (рейсы, логи в чат)
# - Аукцион для пользователей (просмотр, ставки, завершение)
# - Продажа авторитета (выставление, покупка)
# - Мультиплеер 21 (полная игровая логика)
# - Проверка бана во всех хендлерах
# - Команда /smuggle_chat для групп с выводом логов
# - Автоудаление команд
...
async def init_db():
    async with db_pool.acquire() as conn:
        ...
        # Кулдауны боя (устарело, но оставим)
        await conn.execute('''
...
# ==================== ФУНКЦИИ ДЛЯ АВТОМАТИЧЕСКОГО ДОБАВЛЕНИЯ ПОЛЕЙ ====================
...
def user_main_keyboard(is_admin_user=False):
    buttons = [
        ...
        [KeyboardButton(text="💼 Продажа авторитета"), KeyboardButton(text="📦 Контрабанда")],
        [KeyboardButton(text="🏪 Мои бизнесы"), KeyboardButton(text="📊 Статистика")],
    ]
    if is_admin_user:
...
def business_choice_keyboard(businesses):
    kb = []
    for biz in businesses:
        kb.append([InlineKeyboardButton(
            text=f"{biz['name']} (стоимость: {biz['cost_smuggle']} ед. контрабанды)",
            callback_data=f"buy_biz_{biz['id']}"
        )])
...
# Все команды и кнопки для обычных пользователей в личных сообщениях

# ==================== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ КОНТРАБАНДЫ ====================
...
async def start_smuggle_run(user_id: int) -> dict:
    ...
    async with db_pool.acquire() as conn:
        run_id = await conn.fetchval(
            "INSERT INTO smuggle_runs (user_id, end_time) VALUES ($1, $2) RETURNING id",
            user_id, end_time
        )
    return {"run_id": run_id, "end_time": end_time, "duration": duration}

# ==================== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ БИЗНЕСА ====================
...
async def cmd_start(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not exists:
            ...
            await message.answer("✅ Добро пожаловать в Malboro GAME! Ты успешно зарегистрирован.")
        else:
...
async def cmd_help_private(message: types.Message):
    ...
    text = (
        ...
        "🏆 Топ игроков – рейтинг по баксам, репутации, контрабанде\n"
        "🎲 Игры – мультиплеер 21\n"
        "📋 Задания – выполнение заданий\n"
        "🏷 Аукцион – участие в аукционах\n"
        "💼 Продажа авторитета – торговля глобальным авторитетом\n"
        "📦 Контрабанда – контрабандные рейсы\n"
        "🏪 Мои бизнесы – просмотр и сбор дохода с бизнесов\n"
        "⚙️ Админ панель – для администраторов\n\n"
        "Также доступны команды:\n"
        "/smuggle – начать контрабандный рейс\n"
        "/mybusiness – мои бизнесы\n"
        "/buyshop – купить бизнес\n"
        "/top – топ по баксам\n"
        "/top_rep – топ по репутации\n"
        "/top_smuggle – топ по контрабанде"
    )
...
async def profile_handler(message: types.Message):
    ...
    try:
        async with db_pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT balance, reputation, total_spent, negative_balance, joined_date, theft_attempts, theft_success, theft_failed, theft_protected, casino_wins, casino_losses, dice_wins, dice_losses, guess_wins, guess_losses, slots_wins, slots_losses, roulette_wins, roulette_losses, multiplayer_wins, multiplayer_losses, exp, level, strength, agility, defense, global_authority, smuggle_goods, smuggle_success, smuggle_fail FROM users WHERE user_id=$1",
                user_id
            )
        if row:
            (balance, rep, spent, neg, joined, attempts, success, failed, protected,
             cw, cl, dw, dl, gw, gl, sw, sl, rw, rl, mpw, mpl,
             exp, level, strength, agility, defense, global_auth,
             smuggle_goods, smuggle_success, smuggle_fail) = row
...
async def bonus_handler(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        user = await conn.fetchrow("SELECT last_bonus FROM users WHERE user_id=$1", user_id)
        if not user:
            await conn.execute(
        ...
        await update_user_balance(user_id, bonus, conn=conn)
        await conn.execute("UPDATE users SET last_bonus=$1 WHERE user_id=$2", now, user_id)

    phrase = get_random_phrase(BONUS_PHRASES, bonus=bonus)
    await message.answer(phrase)

# ==================== ТОПЫ ====================
@dp.message_handler(lambda message: message.text == "🏆 Топ игроков" or message.text == "/top")
async def top_balance(message: types.Message):
    ...
    for i, row in enumerate(rows, 1):
        text += f"{i}. {row['first_name']} – {row['balance']} баксов\n"
    await message.answer(text)

@dp.message_handler(commands=['top_rep'])
async def top_reputation(message: types.Message):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    if await is_banned(user_id):
    ...
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("SELECT first_name, reputation FROM users ORDER BY reputation DESC LIMIT 10")
    if not rows:
        await message.answer("Нет данных.")
        return
    text = "⭐️ Топ по репутации:\n\n"
    for i, row in enumerate(rows, 1):
        text += f"{i}. {row['first_name']} – {row['reputation']}\n"
    await message.answer(text)

@dp.message_handler(commands=['top_smuggle'])
async def top_smuggle(message: types.Message):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    if await is_banned(user_id):
    ...
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("SELECT first_name, smuggle_goods FROM users ORDER BY smuggle_goods DESC LIMIT 10")
    if not rows:
        await message.answer("Нет данных.")
        return
    text = "📦 Топ контрабандистов (по количеству товара):\n\n"
    for i, row in enumerate(rows, 1):
        text += f"{i}. {row['first_name']} – {row['smuggle_goods']} ед.\n"
    await message.answer(text)
...
async def guess_number(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            ...
            profit = amount * multiplier
            rep = int(await get_setting("guess_reputation"))
            await update_user_balance(user_id, profit, conn=conn)
            await update_user_reputation(user_id, rep)
            exp = int(await get_setting("exp_per_guess_win"))
            phrase = get_random_phrase(GUESS_WIN_PHRASES, secret=secret, profit=profit, rep=rep)
        else:
...
async def slots_bet(message: types.Message, state: FSMContext):
    ...
    combo, multiplier, win = await slots_spin()
    async with db_pool.acquire() as conn:
        await update_user_balance(user_id, -amount, conn=conn)
        ...
        if win:
            ...
            phrase = get_random_phrase(SLOTS_LOSE_PHRASES, combo=combo, loss=amount)
        await add_exp(user_id, exp, conn=conn)
...
# ==================== ХЕНДЛЕРЫ КОНТРАБАНДЫ ====================
@dp.message_handler(lambda message: message.text == "📦 Контрабанда" or message.text == "/smuggle")
async def cmd_smuggle(message: types.Message):
    if message.chat.type != 'private':
        await auto_delete_reply(message, "❌ Эта команда работает только в личных сообщениях.")
        return
    user_id = message.from_user.id
    if await is_banned(user_id):
        return
    ...
    if not ok:
        ...
        await message.answer(f"⏳ Ты ещё не вернулся из рейса. Подожди {minutes} мин {seconds} сек.")
        return
    ...
    run_info = await start_smuggle_run(user_id)
    await set_smuggle_cooldown(user_id, 0)
    
    end_time_str = run_info['end_time'].strftime("%H:%M %d.%m.%Y")
    phrase = get_random_phrase(SMUGGLE_START_PHRASES, end_time=end_time_str)
    await message.answer(phrase)

# ==================== ХЕНДЛЕРЫ БИЗНЕСА ====================
@dp.message_handler(lambda message: message.text == "🏪 Мои бизнесы" or message.text == "/mybusiness")
async def cmd_mybusiness(message: types.Message):
    if message.chat.type != 'private':
        await auto_delete_reply(message, "❌ Эта команда работает только в личных сообщениях.")
        return
    user_id = message.from_user.id
    ...
    async with db_pool.acquire() as conn:
        ...
        businesses = await conn.fetch(
            "SELECT ub.*, bt.name, bt.income_per_hour, bt.max_storage FROM user_businesses ub "
            "JOIN business_types bt ON ub.business_type_id = bt.id WHERE ub.user_id=$1",
            user_id
        )
    if not businesses:
        await message.answer("📭 У тебя пока нет бизнеса. Купи его командой /buyshop")
        return
    ...
    for biz in businesses:
        ...
        storage_bucks = biz['max_storage'] // 100
        storage_cents = biz['max_storage'] % 100
        income_bucks = biz['income_per_hour'] // 100
        income_cents = biz['income_per_hour'] % 100
        
        text += f"🔹 {biz['name']}\n"
        text += f"   Доход: {income_bucks} баксов {income_cents} центов/час\n"
        text += f"   Накоплено: {accum_bucks} баксов {accum_cents} центов из {storage_bucks} баксов {storage_cents} центов\n"
        text += f"   Уровень: {biz['level']}\n\n"
        
        total_accumulated += biz['accumulated']
        kb.add(InlineKeyboardButton(f"💰 Собрать {biz['name']}", callback_data=f"collect_biz_{biz['id']}"))
    
    total_bucks = total_accumulated // 100
    total_cents = total_accumulated % 100
    if total_accumulated > 0:
        text += f"💰 Всего можно собрать: {total_bucks} баксов {total_cents} центов"
    
    await message.answer(text, reply_markup=kb)

@dp.message_handler(lambda message: message.text == "/buyshop")
async def cmd_buyshop(message: types.Message):
    if message.chat.type != 'private':
        await auto_delete_reply(message, "❌ Эта команда работает только в личных сообщениях.")
        return
    user_id = message.from_user.id
    ...
    async with db_pool.acquire() as conn:
        ...
        user_auth = await get_user_global_authority(user_id)
        businesses = await conn.fetch(
            "SELECT * FROM business_types WHERE required_authority <= $1 ORDER BY cost_smuggle",
            user_auth
        )
        available = [b for b in businesses if b['id'] not in owned_ids]
    
    if not available:
        await message.answer("📭 Нет доступных для покупки бизнесов. Возможно, у тебя недостаточно авторитета или все уже куплены.")
        return

    text = "💼 Доступные бизнесы:\n\n"
    for biz in available:
        cost_bucks = biz['cost_smuggle'] // 100
        cost_cents = biz['cost_smuggle'] % 100
        income_bucks = biz['income_per_hour'] // 100
        income_cents = biz['income_per_hour'] % 100
        storage_bucks = biz['max_storage'] // 100
        storage_cents = biz['max_storage'] % 100
        ...
        text += f"   💰 Стоимость: {cost_bucks} баксов {cost_cents} центов (контрабанда)\n"
        text += f"   📈 Доход: {income_bucks} баксов {income_cents} центов/час\n"
        text += f"   🏪 Хранилище: {storage_bucks} баксов {storage_cents} центов\n"
        text += f"   👑 Требуется авторитет: {biz['required_authority']}\n\n"
    
    kb = business_choice_keyboard(available)
    await message.answer(text, reply_markup=kb)
    await BuyBusiness.choosing_type.set()

@dp.callback_query_handler(lambda c: c.data.startswith("buy_biz_"), state=BuyBusiness.choosing_type)
async def buy_business_choose(callback: types.CallbackQuery, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        biz = await conn.fetchrow("SELECT * FROM business_types WHERE id=$1", biz_id)
        if not biz:
            await callback.answer("❌ Бизнес не найден.", show_alert=True)
            await state.finish()
            return
        
        owned = await conn.fetchval("SELECT 1 FROM user_businesses WHERE user_id=$1 AND business_type_id=$2", callback.from_user.id, biz_id)
        if owned:
            await callback.answer("❌ У тебя уже есть этот бизнес.", show_alert=True)
            await state.finish()
            return
        
        user_goods = await conn.fetchval("SELECT smuggle_goods FROM users WHERE user_id=$1", callback.from_user.id)
        if user_goods < biz['cost_smuggle']:
            await callback.answer(f"❌ Недостаточно контрабанды. Нужно {biz['cost_smuggle']}, у тебя {user_goods}.", show_alert=True)
            await state.finish()
            return
        
        user_auth = await get_user_global_authority(callback.from_user.id)
        if user_auth < biz['required_authority']:
            await callback.answer(f"❌ Требуется авторитет {biz['required_authority']}, у тебя {user_auth}.", show_alert=True)
            await state.finish()
            return
        
        await state.update_data(biz_id=biz_id, biz_name=biz['name'], cost=biz['cost_smuggle'])
    
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Подтвердить", callback_data="confirm_buy_yes")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="confirm_buy_no")]
    ])
    await callback.message.edit_text(
        f"Ты собираешься купить бизнес «{biz['name']}» за {biz['cost_smuggle']} ед. контрабанды. Подтверждаешь?",
        reply_markup=kb
    )
    await BuyBusiness.confirming.set()
    await callback.answer()

@dp.callback_query_handler(lambda c: c.data in ["confirm_buy_yes", "confirm_buy_no"], state=BuyBusiness.confirming)
async def buy_business_confirm(callback: types.CallbackQuery, state: FSMContext):
    if callback.data == "confirm_buy_no":
        await state.finish()
        await callback.message.edit_text("❌ Покупка отменена.")
        await callback.answer()
        return
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            user_goods = await conn.fetchval("SELECT smuggle_goods FROM users WHERE user_id=$1", user_id)
            if user_goods < cost:
                await callback.answer("❌ Недостаточно контрабанды. Попробуй снова.", show_alert=True)
                await state.finish()
                return
            
            await conn.execute(
                "UPDATE users SET smuggle_goods = smuggle_goods - $1 WHERE user_id=$2",
                cost, user_id
            )
            await conn.execute(
                "INSERT INTO user_businesses (user_id, business_type_id, last_collection) VALUES ($1, $2, $3)",
                user_id, biz_id, datetime.now()
            )
    
    await callback.message.edit_text(f"✅ Поздравляю! Ты приобрёл бизнес «{data['biz_name']}».")
    await state.finish()
    await callback.answer()

@dp.callback_query_handler(lambda c: c.data.startswith("collect_biz_"))
async def collect_business(callback: types.CallbackQuery):
    biz_id = int(callback.data.split("_")[2])
    user_id = callback.from_user.id
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if biz['accumulated'] == 0:
                await callback.answer("⏳ В этом бизнесе пока нет дохода.", show_alert=True)
                return
            
            amount = biz['accumulated']
            coins = amount // 100
            cents = amount % 100
            ...
            await conn.execute(
                "UPDATE user_businesses SET accumulated=0, last_collection=$1 WHERE id=$2",
                datetime.now(), biz_id
            )
    
    await callback.answer(f"✅ Ты собрал {coins} баксов и {cents} центов!", show_alert=True)
    await cmd_mybusiness(callback.message)

# ==================== ХЕНДЛЕРЫ ДЛЯ КНОПКИ "ИГРЫ" (МУЛЬТИПЛЕЕР) ====================
...
async def games_menu(message: types.Message):
    ...
    if not ok:
        await message.answer("❗️ Сначала подпишись на каналы.", reply_markup=subscription_inline(not_subscribed))
        return
    await message.answer("Мультиплеерные игры (21):", reply_markup=games_menu_keyboard())

# ----- Создание комнаты -----
@dp.message_handler(lambda message: message.text == "👥 Комнатная игра 21")
async def multiplayer_create_start(message: types.Message):
    if message.chat.type != 'private':
        return
...
async def multiplayer_create_max_players(message: types.Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.finish()
    ...
    await message.answer("Введи ставку (целое число):")
    await MultiplayerGame.create_bet.set()

@dp.message_handler(state=MultiplayerGame.create_bet)
async def multiplayer_create_bet(message: types.Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.finish()
        await games_menu(message)
        return
    ...
    deck = ','.join(create_deck())
    async with db_pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO multiplayer_games (game_id, host_id, max_players, bet_amount, deck) VALUES ($1, $2, $3, $4, $5)",
            game_id, message.from_user.id, max_players, bet, deck
        )
        # Добавляем хоста как игрока
        cards = [deck.split(',')[0], deck.split(',')[1]]
        deck_list = deck.split(',')[2:]
        deck = ','.join(deck_list)
        value = calculate_hand_value(cards)
        await conn.execute(
            "INSERT INTO game_players (game_id, user_id, username, cards, value) VALUES ($1, $2, $3, $4, $5)",
            game_id, message.from_user.id, message.from_user.first_name, ','.join(cards), value
        )
        await conn.execute("UPDATE multiplayer_games SET deck=$1 WHERE game_id=$2", deck, game_id)
    await message.answer(
        f"🎮 Комната {game_id} создана! Ставка: {bet} баксов. Макс. игроков: {max_players}.\n"
        f"Поделись этим кодом с друзьями, чтобы они могли присоединиться.",
...
async def multiplayer_list(message: types.Message):
    if message.chat.type != 'private':
        return
    async with db_pool.acquire() as conn:
        rooms = await conn.fetch(
            ...
            "FROM multiplayer_games WHERE status='waiting'"
        )
    if not rooms:
    ...
    kb = InlineKeyboardMarkup(row_width=1)
    for room in rooms:
        text += f"🆔 {room['game_id']} | Ставка: {room['bet_amount']} | Игроков: {room['players_count']}/{room['max_players']}\n"
        kb.add(InlineKeyboardButton(f"Войти в {room['game_id']}", callback_data=f"join_game_{room['game_id']}"))
    await message.answer(text, reply_markup=kb)
...
async def join_game(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not game:
            await callback.answer("Комната не найдена или уже началась.", show_alert=True)
            return
        ...
        if balance < game['bet_amount']:
            await callback.answer("Недостаточно баксов для ставки.", show_alert=True)
            return
        # Добавляем игрока
        ...
        await conn.execute(
            "INSERT INTO game_players (game_id, user_id, username, cards, value) VALUES ($1, $2, $3, $4, $5)",
            game_id, user_id, callback.from_user.first_name, ','.join(cards), value
        )
    ...
    await callback.message.answer(f"✅ Ты в игре {game_id}. Жди начала от хоста.")
...
async def multiplayer_top(message: types.Message):
    async with db_pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT first_name, multiplayer_wins, multiplayer_losses FROM users ORDER BY multiplayer_wins DESC LIMIT 10"
        )
    if not rows:
        await message.answer("Нет данных.")
        return
    ...
    for i, row in enumerate(rows, 1):
        text += f"{i}. {row['first_name']} – побед: {row['multiplayer_wins']}, поражений: {row['multiplayer_losses']}\n"
    await message.answer(text)
...
async def multiplayer_rules(message: types.Message):
    rules = (
        "📖 Правила игры «21» (блэкджек):\n"
        "Цель – набрать 21 очко или быть ближе к 21, чем соперник, но не больше.\n"
        "Карты от 2 до 10 – по номиналу, J/Q/K – 10 очков, Туз – 1 или 11.\n"
        "Действия: «Ещё» – взять карту, «Хватит» – остановиться,\n"
        "«Удвоить» – удвоить ставку и взять ровно одну карту (только на первых двух),\n"
        "«Сдаться» – забрать половину ставки и выйти из игры.\n"
        "Победитель забирает банк, при ничьей – делёжка."
    )
    await message.answer(rules)

# ----- Инлайн-кнопки управления комнатой (хост) -----
...
async def start_game(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        if len(players) < 2:
            ...
            return
        await conn.execute("UPDATE multiplayer_games SET status='active' WHERE game_id=$1", game_id)
        # Списываем ставки со всех игроков
        for player in players:
            await update_user_balance(player['user_id'], -game['bet_amount'], conn=conn)
        # Отправляем сообщение о начале игры
        await callback.message.edit_text(f"Игра {game_id} началась!")
        # Уведомляем всех игроков
        for player in players:
            await safe_send_message(player['user_id'], f"Игра {game_id} началась! Твой ход.")
        # Показываем состояние текущего игрока
        await show_game_state(game_id, conn)
    await callback.answer()

async def show_game_state(game_id: str, conn=None):
    # Функция для отображения состояния игры (будет вызываться после каждого действия)
    # Здесь нужно получить текущего игрока и отправить ему клавиатуру с действиями
    # Реализация будет в части с игровой логикой, но для краткости пока оставим заглушку
    pass

@dp.callback_query_handler(lambda c: c.data == "room_hit")
async def room_hit(callback: types.CallbackQuery):
    # Логика взятия карты
    await callback.answer("Функция в разработке", show_alert=True)

@dp.callback_query_handler(lambda c: c.data == "room_stand")
async def room_stand(callback: types.CallbackQuery):
    await callback.answer("Функция в разработке", show_alert=True)

@dp.callback_query_handler(lambda c: c.data == "room_double")
async def room_double(callback: types.CallbackQuery):
    await callback.answer("Функция в разработке", show_alert=True)

@dp.callback_query_handler(lambda c: c.data == "room_surrender")
async def room_surrender(callback: types.CallbackQuery):
    await callback.answer("Функция в разработке", show_alert=True)

@dp.callback_query_handler(lambda c: c.data == "room_chat")
async def room_chat(callback: types.CallbackQuery):
    await callback.answer("Чат комнаты пока не реализован", show_alert=True)

@dp.callback_query_handler(lambda c: c.data.startswith("close_room_"))
async def close_room(callback: types.CallbackQuery):
    game_id = callback.data.split("_")[2]
    user_id = callback.from_user.id
    async with db_pool.acquire() as conn:
    ...
    await callback.message.edit_text(f"Комната {game_id} закрыта.")
    await callback.answer()
  # ==================== ЧАСТЬ 4: ГРУППОВЫЕ ХЕНДЛЕРЫ ====================
# Все функции для работы в группах: /fight, /gym, /status, /top, /boss_status,
# /smuggle_chat, /help, подгон, обработка добавления бота, атака босса
...
async def cmd_fight(message: types.Message):
    ...
    await auto_delete_reply(
        message,
        f"{username}, {phrase}\n"
        f"Твои очки прокачки в этом чате: {await get_chat_authority(chat_id, user_id)}\n"
        f"Глобальный авторитет: {await get_user_global_authority(user_id)}"
    )
...
async def cmd_gym(message: types.Message):
    ...
    kb.add(
        types.InlineKeyboardButton(f"💪 Сила ({strength_cost} очков)", callback_data="gym_strength"),
        types.InlineKeyboardButton(f"🏃 Ловкость ({agility_cost} очков)", callback_data="gym_agility"),
        types.InlineKeyboardButton(f"🛡 Защита ({defense_cost} очков)", callback_data="gym_defense"),
        types.InlineKeyboardButton("❌ Отмена", callback_data="gym_cancel")
    )

    await auto_delete_reply(
        message,
        f"🏋️ Качалка! У тебя {authority} очков прокачки в этом чате.\n"
...
async def gym_callback(callback: types.CallbackQuery):
    ...
    authority = await get_chat_authority(chat_id, user_id)
    if authority < cost:
        await callback.answer(f"❌ Не хватает очков прокачки. Нужно {cost}, у тебя {authority}.", show_alert=True)
        return
    ...
    if await spend_chat_authority(chat_id, user_id, cost):
        ...
        await callback.message.edit_text(
            f"✅ Ты улучшил {stat}!\n"
            f"Осталось очков прокачки в этом чате: {await get_chat_authority(chat_id, user_id)}"
        )
    else:
        await callback.answer("❌ Ошибка при списании очков.", show_alert=True)
...
async def cmd_status(message: types.Message):
    ...
    text = (
        ...
        f"🎮 Очки прокачки: {chat_auth}\n"
        f"👑 Глобальный авторитет: {global_auth}\n"
...
async def show_chat_top(chat_id: int, message: types.Message, page: int = 1, order: str = "authority"):
    ...
    for i, row in enumerate(rows, start=offset+1):
        ...
        text += f"{i}. {name} – Очки прокачки: {row['authority']}, Урон: {row['total_damage']}, Боёв: {row['fights']}\n"
    ...
    kb.row(
        types.InlineKeyboardButton("📊 По очкам", callback_data=f"chat_top_authority_1"),
...
async def cmd_help_chat(message: types.Message):
    ...
    text = (
        ...
        "/gym – улучшить характеристики за очки прокачки\n"
        "/status – твой статус\n"
        "/top – топ чата\n"
...
# Добавляем колонку chat_id в smuggle_runs (если ещё не добавлена)
async def add_smuggle_chat_column():
    async with db_pool.acquire() as conn:
        try:
            await conn.execute("ALTER TABLE smuggle_runs ADD COLUMN IF NOT EXISTS chat_id BIGINT")
            logging.info("✅ Колонка chat_id добавлена в smuggle_runs")
        except Exception as e:
            logging.error(f"Ошибка при добавлении колонки chat_id: {e}")
...
async def cmd_smuggle_chat(message: types.Message):
    ...
    # Запускаем рейс с указанием chat_id
    min_dur = int(await get_setting("smuggle_min_duration"))
    max_dur = int(await get_setting("smuggle_max_duration"))
    duration = random.randint(min_dur, max_dur)
    end_time = datetime.now() + timedelta(minutes=duration)
    ...
    async with db_pool.acquire() as conn:
        run_id = await conn.fetchval(
            "INSERT INTO smuggle_runs (user_id, end_time, chat_id) VALUES ($1, $2, $3) RETURNING id",
            user_id, end_time, chat_id
        )
    await set_smuggle_cooldown(user_id, 0)
    
    end_time_str = end_time.strftime("%H:%M %d.%m.%Y")
    phrase = get_random_phrase(SMUGGLE_START_PHRASES, end_time=end_time_str)
    await auto_delete_reply(message, f"🚤 {message.from_user.first_name} отправился в контрабандный рейс! Вернётся примерно {end_time_str}.")
...
async def unknown_command(message: types.Message):
  ...
  # ==================== ЧАСТЬ 5: ПОЛЬЗОВАТЕЛЬСКИЕ ХЕНДЛЕРЫ (АУКЦИОН, АВТОРИТЕТ, МАГАЗИН, ЗАДАНИЯ) ====================

# ==================== АУКЦИОН (ПОЛЬЗОВАТЕЛЬСКАЯ ЧАСТЬ) ====================
@dp.message_handler(lambda message: message.text == "🏷 Аукцион")
async def user_auction_menu(message: types.Message):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    if await is_banned(user_id):
    ...
    total, auctions = await get_active_auctions(1)
    if not auctions:
        await message.answer("🏷 На данный момент нет активных аукционов.")
        return

    total_pages = (total + ITEMS_PER_PAGE - 1) // ITEMS_PER_PAGE
    kb = auction_list_keyboard(auctions, 1, total_pages)
    await message.answer("🏷 Активные аукционы:", reply_markup=kb)
...
async def auction_page_callback(callback: types.CallbackQuery):
    ...
    total, auctions = await get_active_auctions(page)
    if not auctions:
        await callback.answer("Аукционы не найдены.", show_alert=True)
        return
    total_pages = (total + ITEMS_PER_PAGE - 1) // ITEMS_PER_PAGE
    kb = auction_list_keyboard(auctions, page, total_pages)
    await callback.message.edit_text("🏷 Активные аукционы:", reply_markup=kb)
    await callback.answer()
...
async def auction_view_callback(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not auction or auction['status'] != 'active':
            await callback.answer("Аукцион не найден или уже завершён.", show_alert=True)
            return
        # Получаем последнюю ставку
        last_bid = await conn.fetchrow(
            "SELECT user_id, bid_amount FROM auction_bids WHERE auction_id=$1 ORDER BY bid_time DESC LIMIT 1",
            auction_id
        )
    text = (
    ...
    if auction['end_time']:
        text += f"⏳ Окончание: {auction['end_time'].strftime('%d.%m.%Y %H:%M')}\n"
    if auction['target_price']:
        text += f"🎯 Целевая цена: {auction['target_price']}\n"
    if last_bid:
        text += f"👤 Последняя ставка: {last_bid['bid_amount']} от пользователя {last_bid['user_id']}\n"
    else:
        text += "👤 Пока нет ставок.\n"

    kb = auction_detail_keyboard(auction_id)
    if auction['photo_file_id']:
        await callback.message.delete()
        await callback.message.answer_photo(
            auction['photo_file_id'],
            caption=text,
            reply_markup=kb
        )
    else:
...
async def auction_bid_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not auction:
            ...
            return
    await state.update_data(auction_id=auction_id, current_price=auction['current_price'])
    await callback.message.answer(
        f"Введи сумму ставки (текущая цена: {auction['current_price']}, минимальный шаг: {await get_setting('auction_min_bid_step')}):",
...
async def auction_bid_amount(message: types.Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.finish()
        await user_auction_menu(message)
        return
    try:
        amount = int(message.text)
    except ValueError:
    ...
    if amount <= current_price:
        await message.answer(f"❌ Ставка должна быть больше текущей цены ({current_price}).")
        return

    success, msg = await place_bid(auction_id, user_id, amount)
    await message.answer(msg)
    if success:
        # Уведомляем предыдущего лидера, если он был
        async with db_pool.acquire() as conn:
            prev_leader = await conn.fetchval(
                "SELECT user_id FROM auction_bids WHERE auction_id=$1 ORDER BY bid_time DESC LIMIT 1 OFFSET 1",
                auction_id
            )
            if prev_leader and prev_leader != user_id:
                await safe_send_message(prev_leader, f"⚠️ Твою ставку на аукционе {auction_id} перебили!")
        # Если аукцион завершился досрочно (целевая цена)
        if "выиграл" in msg:
            await user_auction_menu(message)
        await state.finish()
    else:
...
async def auction_list_back(callback: types.CallbackQuery):
    await callback.message.delete()
    await user_auction_menu(callback.message)
...
async def authority_shop_menu(message: types.Message):
    ...
    if not ok:
        await message.answer("❗️ Сначала подпишись на каналы.", reply_markup=subscription_inline(not_subscribed))
        return
    await message.answer("💼 Торговля авторитетом:", reply_markup=authority_shop_keyboard())

@dp.message_handler(lambda message: message.text == "💰 Продать авторитет")
async def sell_authority_start(message: types.Message):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    if await is_banned(user_id):
    ...
    if not ok:
        await message.answer("❗️ Сначала подпишись на каналы.", reply_markup=subscription_inline(not_subscribed))
        return
    await message.answer("Введи количество авторитета для продажи:", reply_markup=back_keyboard())
...
async def sell_authority_amount(message: types.Message, state: FSMContext):
    ...
    if auth < amount:
        await message.answer(f"❌ У тебя только {auth} авторитета.")
        return
    await state.update_data(amount=amount)
    await message.answer("Введи цену за одну единицу авторитета (в баксах):")
...
async def sell_authority_price(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        min_price = int(await get_setting("min_authority_price"))
        if price < min_price:
            await message.answer(f"❌ Минимальная цена за единицу: {min_price}.")
            return
    except ValueError:
        await message.answer("❌ Введи целое число.")
        return
    ...
    offer_id = await create_authority_offer(user_id, amount, price)
    if offer_id:
        await message.answer(get_random_phrase(AUTHORITY_SELL_PHRASES, amount=amount, price=price, offer_id=offer_id))
    else:
        await message.answer("❌ Не удалось создать предложение. Возможно, недостаточно авторитета.")
    await state.finish()
...
async def buy_authority_list(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        offers = await conn.fetch(
            "SELECT * FROM authority_offers WHERE status='active' ORDER BY price_per_unit"
        )
    if not offers:
        await message.answer("📭 Нет активных предложений на продажу.")
        return
    kb = authority_offers_keyboard(offers)
    await message.answer("📋 Доступные предложения:", reply_markup=kb)

@dp.callback_query_handler(lambda c: c.data.startswith("buy_offer_"))
async def buy_authority_confirm(callback: types.CallbackQuery):
    offer_id = int(callback.data.split("_")[2])
    user_id = callback.from_user.id

    success, msg = await buy_authority(user_id, offer_id)
    await callback.answer(msg, show_alert=True)
    if success:
        await callback.message.delete()
        await authority_shop_menu(callback.message)

@dp.callback_query_handler(lambda c: c.data == "authority_back")
async def authority_back(callback: types.CallbackQuery):
    await callback.message.delete()
    await authority_shop_menu(callback.message)
...
async def my_offers(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        offers = await conn.fetch(
            "SELECT * FROM authority_offers WHERE seller_id=$1 AND status='active'",
            user_id
        )
    if not offers:
...
async def shop_list(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        items = await conn.fetch("SELECT * FROM shop_items ORDER BY id")
    if not items:
    ...
    kb = InlineKeyboardMarkup(row_width=1)
    for item in items:
        text = f"{item['name']} – {item['price']} баксов"
        if item['stock'] != -1:
            text += f" (осталось: {item['stock']})"
        kb.add(InlineKeyboardButton(text, callback_data=f"buy_item_{item['id']}"))
...
async def buy_item(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        item = await conn.fetchrow("SELECT * FROM shop_items WHERE id=$1", item_id)
        if not item:
        ...
        if balance < item['price']:
            await callback.answer("Недостаточно баксов.", show_alert=True)
            return

        async with conn.transaction():
            await update_user_balance(user_id, -item['price'], conn=conn)
            if item['stock'] > 0:
            ...
            await update_user_total_spent(user_id, item['price'])

    await callback.answer("✅ Покупка совершена! Ожидай, админ скоро свяжется.", show_alert=True)
    phrase = random.choice(PURCHASE_PHRASES)
    await callback.message.answer(phrase)

    # Уведомление в чаты о крупной покупке
    if item['price'] >= BIG_PURCHASE_THRESHOLD and await get_setting("chat_notify_big_purchase") == "1":
        await notify_chats(f"🛒 {callback.from_user.first_name} купил {item['name']} за {item['price']} баксов!")
...
async def promo_activate(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        used = await conn.fetchval("SELECT 1 FROM promo_activations WHERE user_id=$1 AND promo_code=$2", user_id, code)
        if used:
        ...
        async with conn.transaction():
            await update_user_balance(user_id, promo['reward'], conn=conn)
    ...
    await message.answer(f"✅ Промокод активирован! Ты получил {promo['reward']} баксов.")
    await state.finish()
...
async def tasks_list(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        # Активные задания, которые пользователь ещё не выполнил или выполнил, но можно повторять
        tasks = await conn.fetch("""
            SELECT t.*,
                   (SELECT status FROM user_tasks WHERE user_id=$1 AND task_id=t.id) as user_status
            FROM tasks t
            WHERE t.active = TRUE
            ORDER BY t.id
        """, user_id)
    ...
    kb = InlineKeyboardMarkup(row_width=1)
    for task in tasks:
        status = "✅" if task['user_status'] == 'completed' else "⏳"
        text += f"{status} <b>{task['name']}</b>\n{task['description']}\n"
        text += f"Награда: {task['reward_coins']} баксов, {task['reward_reputation']} репутации\n"
        if task['user_status'] != 'completed':
            kb.add(InlineKeyboardButton(f"Выполнить {task['name']}", callback_data=f"do_task_{task['id']}"))
        text += "\n"
...
async def do_task(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        # Проверяем, не выполнил ли уже пользователь
        existing = await conn.fetchval("SELECT status FROM user_tasks WHERE user_id=$1 AND task_id=$2", user_id, task_id)
        ...
        if task['task_type'] == 'subscribe':
            ...
            try:
                member = await bot.get_chat_member(chat_id=int(task['target_id']), user_id=user_id)
                if member.status in ['left', 'kicked']:
                    await callback.answer("❌ Ты не подписан на этот канал.", show_alert=True)
                    return
                # Если требуется проверка по дням
                if task['required_days'] > 0:
                    # Здесь нужна логика проверки, сколько дней пользователь подписан
                    # Упрощённо: считаем, что если он подписан сейчас, то задание выполнено
                    # Для полноценной проверки нужно хранить дату подписки, но мы пропустим
                    pass
            except Exception as e:
        ...
        async with conn.transaction():
            ...
            expires_at = None
            if task['penalty_days'] > 0:
                expires_at = datetime.now() + timedelta(days=task['penalty_days'])
            await conn.execute(
    ...
    await callback.answer(f"✅ Задание выполнено! Ты получил {task['reward_coins']} баксов и {task['reward_reputation']} репутации.", show_alert=True)
    await tasks_list(callback.message)
  # ==================== ЧАСТЬ 6: АДМИНИСТРАТИВНЫЕ ХЕНДЛЕРЫ (ЧАСТЬ 1) ====================
# Управление пользователями, магазином, розыгрышами, каналами, промокодами
...
async def promos_page_callback(callback: types.CallbackQuery):
    ...
    await callback.answer()
  # ==================== ЧАСТЬ 7: АДМИНИСТРАТИВНЫЕ ХЕНДЛЕРЫ (ЧАСТЬ 2) ====================
# Управление заданиями, чатами, боссами, аукционами, рекламой
...
async def list_active_bosses(message: types.Message):
    ...
    kb = InlineKeyboardMarkup(row_width=1)
    for row in rows:
        text += f"ID {row['id']}: {row['name']} (ур. {row['level']})\n"
        text += f"   {row['description'] or 'Без описания'}\n"
        text += f"   HP: {row['hp']}/{row['max_hp']}, награда: {row['reward_coins']}\n"
...
async def delete_ad_callback(callback: types.CallbackQuery):
    ...
    await callback.message.delete()
  # ==================== ЧАСТЬ 8: АДМИНИСТРАТИВНЫЕ ХЕНДЛЕРЫ (ЧАСТЬ 3) ====================
# Статистика, блокировки, рассылка, очистка, настройки, управление админами
...
async def stats_handler(message: types.Message):
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            total_smuggle_goods = await conn.fetchval("SELECT SUM(smuggle_goods) FROM users") or 0
        text = (
            f"📊 Статистика:\n"
            ...
            f"📦 Всего контрабанды: {total_smuggle_goods} ед."
        )
        permissions = await get_admin_permissions(message.from_user.id)
...
async def settings_smuggle(message: types.Message, state: FSMContext):
    if not await has_permission(message.from_user.id, "edit_settings"):
        return
    params = [
        ...
        ("smuggle_authority_multiplier", "📈 Множитель от авторитета (0.1 = +10% за единицу)"),
        ("smuggle_cooldown_minutes", "⏳ Кулдаун между рейсами (мин)"),
        ("smuggle_fail_penalty_minutes", "➕ Доп. кулдаун при провале (мин)"),
        ("cleanup_days_smuggle", "🧹 Дней хранения завершённых рейсов"),
    ]
    kb = InlineKeyboardMarkup(row_width=1)
    for key, desc in params:
        value = await get_setting(key)
        kb.add(InlineKeyboardButton(f"{desc}: {value}", callback_data=f"edit_{key}"))
    kb.add(InlineKeyboardButton("◀️ Назад", callback_data="settings_back"))
    await message.answer("⚙️ Настройки контрабанды (нажми на параметр для изменения):", reply_markup=kb)
...
# Раздел "Помощники" ведёт сюда же
@dp.message_handler(lambda message: message.text == "⚔️ Помощники")
...
async def reject_chat_callback(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        await safe_send_message(request['requested_by'], f"❌ Запрос на активацию чата «{request['title']}» отклонён.")
    await callback.answer()
  # ==================== ЧАСТЬ 9: ФОНОВЫЕ ЗАДАЧИ, ОЧИСТКА, ЗАПУСК ====================
    ...
    while True:
        ...
        try:
            async with db_pool.acquire() as conn:
                ...
                for run in runs:
                    user_id = run['user_id']
                    chat_id = run.get('chat_id')  # может быть None (личка)
                    ...
                    if chat_id:
                        # Если рейс был из чата, отправляем в чат
                        try:
                            await bot.send_message(chat_id, f"🚤 {notify_text}")
                        except:
...
async def on_startup(dp):
    ...
    await add_smuggle_chat_column()  # добавляем колонку chat_id в smuggle_runs
