...
async def process_roulette_bet(message: types.Message, state: FSMContext):
    ...
    await message.answer(phrase)
    await state.finish()
# ==================== ЧАСТЬ 5: ПОЛЬЗОВАТЕЛЬСКИЕ ХЕНДЛЕРЫ (БИЗНЕСЫ, АУКЦИОН, АВТОРИТЕТ, МАГАЗИН, ПРОМОКОДЫ, ЗАДАНИЯ) ====================
...
async def tasks_list(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        # Активные задания, которые пользователь ещё не выполнил или выполнил, но можно повторять (с учётом max_completions)
...
async def do_task(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        # Проверяем, не превышен ли лимит выполнений
        ...
        if task['task_type'] == 'subscribe':
            ...
            try:
                ...
                if member.status in ['left', 'kicked']:
                    ...
                    return
                # Если требуется проверка по дням (упрощённо: считаем, что если подписан, то ок)
                if task['required_days'] > 0:
                    # Здесь можно добавить проверку даты подписки, но пока пропускаем
                    pass
            except Exception as e:
...
async def cmd_fight(message: types.Message):
    ...
    # Добавляем авторитет в текущем чате (очки прокачки)
...
async def cmd_gym(message: types.Message):
    ...
    chat_id = message.chat.id
    user_id = message.from_user.id
    # Используем очки прокачки именно в этом чате
...
# ==================== ПОЛНЫЙ РАБОЧИЙ БОТ (ИТОГОВАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ) – ЧАСТЬ 7 ====================
# Административные хендлеры: пользователи, магазин, розыгрыши, каналы, промокоды, задания, чаты, боссы
