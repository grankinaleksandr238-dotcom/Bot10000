...
async def finish_giveaway_winners(message: types.Message, state: FSMContext):
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            # Удаляем старые завершённые розыгрыши, оставляем только последние MAX_COMPLETED_GIVEAWAYS
            await conn.execute("""
...
async def delete_boss_by_id_start(message: types.Message):
    ...
    await BossSpawn.chat_id.set()  # переиспользуем состояние, но сохраним как boss_id
    # Но у нас нет состояния для этого, поэтому создадим временное
    # Проще сделать отдельный хендлер, но для простоты можно просто спросить ID и удалить
    # Однако мы уже сделали удаление через инлайн-кнопки, этого достаточно.
...
async def settings_theft(message: types.Message, state: FSMContext):
    ...
    for key, desc in params:
        ...
        text += f"{desc}: {value}\n"
        kb.add(InlineKeyboardButton(f"✏️ {desc}", callback_data=f"edit_{key}"))
...
async def smuggle_check_loop():
    """Проверяет завершённые контрабандные рейсы и начисляет награду с историей."""
    while True:
            ...
            async with db_pool.acquire() as conn:
                ...
                for run in runs:
                    ...
                    if rand <= success_chance:
                        ...
                        # Добавляем штрафной кулдаун с использованием INSERT ... ON CONFLICT
                        penalty_time = datetime.now() + timedelta(minutes=penalty)
                        await conn.execute('''
                            INSERT INTO global_cooldowns (user_id, command, last_used)
                            VALUES ($1, $2, $3)
                            ON CONFLICT (user_id, command) DO UPDATE SET last_used = $3
...
async def on_startup(dp):
    ...
    await init_db()
    # Дополнительная проверка наличия всех колонок (включая last_boss_status_time)
    async with db_pool.acquire() as conn:
