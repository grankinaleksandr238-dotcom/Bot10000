...
# - Поля smuggle_goods, smuggle_success, smuggle_fail в таблицу users
# - Таблицы smuggle_runs, business_types, user_businesses
# - Новые настройки для контрабанды в DEFAULT_SETTINGS
# - Функции add_smuggle_columns и create_smuggle_tables для инициализации
# - Гангстерские имена и фразы для боссов (заменены)
...
from aiogram.utils.exceptions import (
    ...
    TerminatedByOtherGetUpdates  # <-- исправлено: добавлен импорт
)
...
# Значения по умолчанию для настроек (расширены для слотов, рулетки и контрабанды)
DEFAULT_SETTINGS = {
    # Кража
    ...
    # Казино и игры (минимальная ставка 50, отдача 25%)
    ...
    # ========== НОВЫЕ НАСТРОЙКИ ДЛЯ КОНТРАБАНДЫ ==========
...
async def init_db():
    async with db_pool.acquire() as conn:
        # Таблица users (с добавленными полями для контрабанды)
        await conn.execute('''
        ...
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS bosses (
                ...
                image_file_id TEXT,
                description TEXT   -- новое поле для описания
            )
        ''')
        ...
        # ========== НОВЫЕ ТАБЛИЦЫ ДЛЯ КОНТРАБАНДЫ И БИЗНЕСА ==========
        # Таблица текущих контрабандных рейсов
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS smuggle_runs (
                ...
                result TEXT,                        -- success, caught, lost
        ...
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS business_types (
                ...
                income_per_hour INTEGER NOT NULL,   -- доход в центах в час
                max_storage INTEGER NOT NULL,        -- максимальное накопление в центах
                required_authority INTEGER DEFAULT 0
            )
        ''')

        # Таблица бизнесов, принадлежащих пользователям
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS user_businesses (
                ...
                accumulated INTEGER DEFAULT 0,       -- накоплено центов
...
# ==================== ФУНКЦИИ ДЛЯ БОССОВ (ГАНГСТЕРСКИЙ СТИЛЬ) ====================
# Новые списки имён, описаний и фраз
BOSS_NAMES = [
...
async def spawn_boss(chat_id: int, level: int = None, image_file_id: str = None):
    """Спавнит босса в указанном чате с гангстерским именем и описанием."""
    if level is None:
...
# ==================== ДОПОЛНИТЕЛЬНЫЕ ФУНКЦИИ (progress_bar и др.) ====================
...
# ==================== В ЭТОЙ ВЕРСИИ ДОБАВЛЕНО ====================
# - Новые состояния для покупки бизнеса (BuyBusiness)
# - Кнопки "Контрабанда" и "Мои бизнесы" в главном меню
# - Клавиатура для выбора бизнеса при покупке
# - Текстовые фразы для контрабанды и бизнеса
...
def user_main_keyboard(is_admin_user=False):
    """Главное меню пользователя с добавленными кнопками контрабанды и бизнеса."""
    buttons = [
...
def business_choice_keyboard(businesses):
    """Создаёт инлайн-клавиатуру со списком доступных бизнесов."""
    kb = []
...
def settings_reply_keyboard():
    buttons = [
        ...
        [KeyboardButton(text="⚙️ Контрабанда")],   # новая кнопка
...
# Гангстерские фразы для боссов (уже использованы в части 1, но оставим для справки)
BOSS_SPAWN_PHRASES = [
    "⚠️ ВНИМАНИЕ! В чате появился {name} (Уровень {level})! {description}",
    "👾 Криминальный авторитет {name} засветился! Уровень {level}, {description}",
    "🔥 Легендарный гангстер {name} объявился! Уровень {level}, {description}",
]
...
# ========== НОВЫЕ ФРАЗЫ ДЛЯ КОНТРАБАНДЫ И БИЗНЕСА ==========
...
# ==================== В ЭТОЙ ВЕРСИИ ДОБАВЛЕНО ====================
# - Команда /start и обработка регистрации
# - Главное меню и навигация
# - Команда /help в личке
# - Ежедневный бонус
# - Все игровые хендлеры (казино, кости, угадай число, слоты, рулетка)
# - Команда /smuggle и обработка контрабандных рейсов
# - Команды /mybusiness и /buyshop для бизнес-системы
# - Обновлённый профиль с отображением статистики контрабанды
# - Вспомогательные функции для бизнеса (обновление дохода)
# - Обработчики инлайн-кнопок для покупки бизнеса и сбора дохода

# ==================== ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ КОНТРАБАНДЫ ====================
async def get_smuggle_cooldown(user_id: int) -> Tuple[bool, int]:
    """Проверяет, может ли пользователь начать рейс. Возвращает (можно ли, оставшиеся секунды)."""
...
async def set_smuggle_cooldown(user_id: int, penalty: int = 0):
    """Устанавливает кулдаун на рейс с возможным дополнительным штрафом."""
...
async def start_smuggle_run(user_id: int) -> dict:
    """Создаёт запись о рейсе и возвращает информацию."""
...
async def update_business_income(user_id: int, conn=None):
    ...
    Вызывается перед просмотром или сбором.
    Если передан conn, использует его (для транзакций).
    """
    async def _update(conn):
        now = datetime.now()
...
async def casino_bet(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            ...
            phrase = random.choice(CASINO_WIN_PHRASES).format(win=amount*multiplier, profit=profit)
            # Уведомление в чаты при крупном выигрыше
...
async def guess_number(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            ...
            phrase = random.choice(GUESS_WIN_PHRASES).format(secret=secret, profit=profit, rep=rep)
        else:
...
async def slots_bet(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            ...
            phrase = random.choice(SLOTS_WIN_PHRASES).format(combo=combo, multiplier=multiplier, profit=profit)
        else:
            exp = int(await get_setting("exp_per_slots_lose"))
            phrase = random.choice(SLOTS_LOSE_PHRASES).format(combo=combo, loss=amount)
        await add_exp(user_id, exp, conn=conn)
...
async def process_roulette_bet(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            ...
            phrase = random.choice(ROULETTE_WIN_PHRASES).format(number=number, color=color, profit=profit)
        else:
            exp = int(await get_setting("exp_per_roulette_lose"))
            phrase = random.choice(ROULETTE_LOSE_PHRASES).format(number=number, color=color, loss=amount)
        await add_exp(user_id, exp, conn=conn)
...
async def cmd_mybusiness(message: types.Message):
    ...
    total_accumulated = 0
    for biz in businesses:
        # Пересчитываем центы в баксы для отображения
    ...
    # Если есть накопления, показываем общую сумму
...
async def cmd_buyshop(message: types.Message):
    ...
    # Получаем список доступных бизнесов (которые пользователь ещё не купил)
    async with db_pool.acquire() as conn:
        # Проверяем, какие бизнесы уже есть
        ...
        # Получаем все типы бизнесов, которые не куплены и доступны по авторитету
...
async def buy_business_confirm(callback: types.CallbackQuery, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            # Проверяем ещё раз наличие контрабанды (на случай, если изменилось)
...
async def collect_business(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            # Переводим баксы на баланс
            await update_user_balance(user_id, coins, conn=conn)
            
            # Сбрасываем накопление и обновляем время последнего сбора
            await conn.execute(
    ...
    # Обновляем сообщение с бизнесами
...
# Примечание: полная реализация мультиплеера будет в отдельной части из-за объёма.
# Здесь мы добавим заглушки для демонстрации.

@dp.message_handler(lambda message: message.text == "👥 Комнатная игра 21")
async def multiplayer_create(message: types.Message):
...
# - В команде /boss_status добавлено отображение описания босса
# - Обновлены фразы боссов на гангстерские (уже в части 1, но здесь используется)
...
# ==================== В ЭТОЙ ВЕРСИИ ИЗМЕНЕНО ====================
# - В поиске пользователя добавлены поля контрабанды
# - Экспорт пользователей теперь включает новые поля автоматически
# - В управлении боссами добавлено отображение описания (необязательно)
...
# ==================== В ЭТОЙ ВЕРСИИ ДОБАВЛЕНО ====================
# - Хендлер раздела настроек "⚙️ Контрабанда" со списком параметров
# - Все параметры контрабанды можно редактировать через универсальный edit_setting_callback
# - Остальные хендлеры (аукцион, реклама, статистика, блокировки, рассылка, настройки, управление админами) обновлены с учётом новых реалий
...
# ==================== В ЭТОЙ ВЕРСИИ ДОБАВЛЕНЫ ВСЕ НЕДОСТАЮЩИЕ ФУНКЦИИ ====================
            ...
            async with db_pool.acquire() as conn:
                ...
                for run in runs:
                    ...
                    if rand <= success_chance:
                        ...
                        # Кулдаун уже установлен при старте, но если нужно, можно обновить время
...
async def on_startup(dp):
    ...
    await add_smuggle_columns()          # из части 1
    await create_smuggle_tables()        # из части 1
