# ==================== ЧАСТЬ 1: ЯДРО И ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ====================
import asyncio
import logging
import random
import os
...
# Добавляем sslmode=require, если его нет (для Railway)
...
# ==================== ЧАСТЬ 2: ПОЛЬЗОВАТЕЛЬСКИЕ И ГРУППОВЫЕ ХЕНДЛЕРЫ ====================
import asyncio
import logging
import random
from datetime import datetime, timedelta, date
...
from core import (
    ...
    notify_chats, safe_send_message, safe_send_chat,
    ...
    spawn_boss, finish_boss_fight,
    # Клавиатуры
    user_main_keyboard, subscription_inline, back_keyboard,
    theft_choice_keyboard, games_keyboard, room_menu_keyboard,
    ...
    MultiplayerGame, RoomChat, CreateGiveaway, AddChannel,
    RemoveChannel, AddShopItem, RemoveShopItem, EditShopItem,
    CreatePromocode, Broadcast, AddBalance, RemoveBalance,
    AddReputation, RemoveReputation, AddExp, SetLevel,
    FindUser, AddJuniorAdmin, RemoveJuniorAdmin,
    CompleteGiveaway, BlockUser, UnblockUser,
    EditSettings, CreateTask, DeleteTask,
    ManageChats, BossSpawn,
    CreateAuction, AuctionBid, CancelAuction,
    CreateAd,
    # Константы и фразы
    ITEMS_PER_PAGE, BIG_WIN_THRESHOLD, BIG_PURCHASE_THRESHOLD,
    MIN_PLAYERS, MAX_PLAYERS, MIN_BET,
    BONUS_PHRASES, CASINO_WIN_PHRASES, CASINO_LOSE_PHRASES,
    PURCHASE_PHRASES, THEFT_CHOICE_PHRASES, THEFT_COOLDOWN_PHRASES,
    ...
    BOSS_SPAWN_PHRASES, BOSS_HIT_PHRASES, BOSS_MISS_PHRASES,
    BOSS_DEATH_PHRASES,
    FIGHT_HIT_PHRASES, FIGHT_CRIT_PHRASES, FIGHT_MISS_PHRASES, FIGHT_COUNTER_PHRASES,
)
from utils import (
    export_users_to_csv, export_table_to_csv,
    format_time_remaining, get_random_phrase,
    calculate_fight_damage, calculate_fight_authority,
    is_critical, is_counter
)

# ==================== ЛИЧНЫЕ СООБЩЕНИЯ (USER) ====================
...
# ----- ОБРАБОТКА НЕИЗВЕСТНЫХ СООБЩЕНИЙ В ЛИЧКЕ -----
@dp.message_handler()
...
# ----- ПОДГОН (GIFT)ОДГОН (GIFT) -----
@dp.message_handler(lambda message: message.chat -----
...
async def chat_gift(message: types.Message):
    chat_id = message.chat.id
.type != 'private' and message.text == "🎁 Подгон")
async def chat_gift(message: types.Message):
    chat_id = message.chat.id
    user_id = message.from_user.id
       user_id = message.from_user.id
...
_limit_per_chat = int(await get_setting("gift_limit_per_day"))
    gift_global_limit = int(await get_setting("g    gift_global_limit = int(await get_setting("gift_global_limit_per_user"))
    gift_cooldown = int(await get_setting("gift_cooldown"))
ift_global_limit_per_user"))
    gift_cooldown = int(await get_setting("gift_cooldown"))
    ...
    async with db_pool.acquire async with db_pool.acquire() as conn:
        chat_info = await conn.fetchrow("SELECT * FROM confirmed_chats WHERE chat_id=$1", chat_id)
() as conn:
        chat_info = await conn.fetchrow("SELECT * FROM confirmed_chats WHERE chat_id=$1", chat_id)
        if        if not chat_info:
            return
        last_gift_date = chat_info['last_g not chat_info:
            return
        last_gift_date = chat_info['last_gift_date']
        gift_count_today = chat_info['ift_date']
        gift_count_today = chat_info['gift_count_today'] if last_gift_date == today else 0
        if gift_count_today >=gift_count_today'] if last_gift_date == today else 0
        if gift_count_today >= gift_limit gift_limit_per_chat:
            await message.reply(f"❌ Сегодня в этом ча_per_chat:
            await message.reply(f"❌ Сегодня в этом чате уже использовано {gift_count_today} изте уже использовано {gift_count_today} из {gift_limit_per_chat} подгонов.")
            return

        user = await conn.fetchrow("SELECT last_gift_time, gift {gift_limit_per_chat} подгонов.")
            return_count_today FROM users WHERE user_id=$1", user_id)


        user = await conn.fetchrow("SELECT last_gift_time, gift_count_today FROM users WHERE user_id=$1", user_id)
        if not user:
            await conn.execute(
                "INSERT INTO        if not user:
            await conn.execute(
                " users (user_id, username, first_name, joined_date) VALUES ($1INSERT INTO users (user_id, username, first_name, joined_date) VALUES ($1, $2, $3, $4) ON CON, $2, $3, $4) ON CONFLICT DO NOTHING",
                user_id, message.from_user.username, message.from_user.firstFLICT DO NOTHING",
                user_id, message.from_user.username, message.from_user.first_name, datetime.now().strftime("%Y-%m-%_name, datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            )
            user = {'d %H:%M:%S")
            )
            user = {'last_gift_time': None, 'gift_count_today':last_gift_time': None, 'gift_count_today': 0 0}
        if user['last_gift_time']:
            user_last_gift}
        if user['last_gift_time']:
            user_last_gift = user['last_gift_time']
            user_gift_count = = user['last_gift_time']
            user_gift user['gift_count_today'] if user_last_gift.startswith(t_count = user['gift_count_today'] if user_last_gift.startswith(today) else 0
        else:
            user_gift_countoday) else 0
        else:
            user_gift_count = 0
        if user_gift_count >= gift_global_limit:
            await message.re = 0
        if user_gift_count >= gift_global_limit:
            await message.reply(f"❌ Сегодня ты уже получиply(f"❌ Сегодня ты уже получил {user_gift_count} из {gift_global_limitл {user_gift_count} из {gift_global_limit} подгонов во всех чатах.")
            return

        if user['} подгонов во всех чатах.")
            return

       last_gift_time']:
            last_gift = datetime.strptime(user if user['last_gift_time']:
            last_gift = datetime.strptime(user['last_gift_time'], "%Y-%m-%d['last_gift_time'], "%Y-%m-%d %H %H:%M:%S")
            diff = (now - last_gift).total:%M:%S")
            diff = (now - last_gift).total_seconds() / 60
            if diff < gift_cooldown:
                remaining = int(gift_seconds() / 60
            if diff < gift_cooldown:
                remaining = int(gift_cooldown - diff)
                await message.reply(f"_cooldown - diff)
                await message.reply(f"⏳ Подгон можно будет использовать через {remaining⏳ Подгон можно будет использовать через {remaining} мин.")
                return

        try:
            admins = await bot.get_chat_administ} мин.")
                return

        try:
            admins = await bot.get_chat_administrators(rators(chat_id)
            eligible = [a.user for a in adminschat_id)
            eligible = [a.user for a in admins if a.user.id != user_id and not await is_banned(a.user.id)]
            if not eligible:
 if a.user.id != user_id and not await is_banned(a.user.id)]
            if not eligible:
                await                await message.reply("❌ Нет подходя message.reply("❌ Нет подходящих получателей для подарка.")
                return
            recipient =щих получателей для подарка.")
                return
            recipient = random.choice(eligible)
        except Exception as e:
            logging.error(f"Gift error: random.choice(eligible)
        except Exception as e:
        ...
        await conn❌ Не удалось выбрать получателя.")
            return

        await conn.execute(
            "INSERT INTO users (user_id, username, first_name, joined_date.execute(
            "INSERT INTO users (user_id, username, first_name,) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING",
            joined_date) VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING",
            recipient.id, recipient.username, recipient.first_name, datetime.now().str recipient.id, recipient.username, recipient.first_name, datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        )
        await update_user_balance(recipient.id, gift_amount,ftime("%Y-%m-%d %H:%M:%S")
        )
        ...
        if last_gift_date == today:
            await conn.execute(" last_gift_date == today:
            await conn.execute("UPDATE confirmed_chats SET gift_count_today = gift_count_todayUPDATE confirmed_chats SET gift_count_today = gift_count_today + 1 WHERE chat_id=$1", chat_id)
 + 1 WHERE chat_id=$1", chat_id)
        else:
            await conn.execute("UPDATE confirmed_chats SET last_gift        else:
            await conn.execute("UPDATE confirmed_chats SET last_gift_date=$_date=$1, gift_count_today=1 WHERE chat_id=$2",1, gift_count_today=1 WHERE chat_id=$2", today, chat_id)

        if user['last_gift today, chat_id)

        if user['last_gift_time'] and user['last_gift_time'].startswith(today):
           _time'] and user['last_gift_time'].startswith(today):
            await conn.execute("UPDATE users SET gift await conn.execute("UPDATE users SET gift_count_today = gift_count_today + 1, last_g_count_today = gift_count_today + 1, last_gift_time=$1ift_time=$1 WHERE user_id=$2",
                               now.strftime("%Y-%m-%d WHERE user_id=$2",
                               now.strftime("%Y-%m-%d %H:%M:%S"), user_id)
        else:
            await conn %H:%M:%S"), user_id)
        else:
            await conn.execute("UPDATE users SET gift_count_today=1, last_gift_time=$1 WHERE user_id=$2",
.execute("UPDATE users SET gift_count_today=1, last_gift_time=$1 WHERE user_id=$2",
                               now.strftime("%Y-%m-%d %H                               now.strftime("%Y-%m-%d %H:%M:%S"), user_id)

    await message.:%M:%S"), user_id)

    await message.answer(
        f"🎁 {message.from_user.first_name} активировал подгонanswer(
        f"🎁 {message.from_user.first_name} активировал подгон!\n"
        f"Счастливчик: {recipient.first_name} получает {gift_amount} монет! 🎉\n!\n"
        f"Счастливчик: {recipient.first_name} получает {gift_amount} монет!"
        f"📊 Сегодня в этом чате осталось подгонов: {g 🎉\n"
        f"📊 Сегодня в этом чате осталось подгонов: {gift_limit_per_chat - (gift_count_today + ift_limit_per_chat - (gift_count_today + 1)}"
    )

# ----- ОБРАБОТЧИК ДО1)}"
    )

# ----- ОБРАБОТЧИК ДОБАВЛЕНИЯ БОТА В ЧАТ -----БАВЛЕНИЯ БОТА В ЧАТ -----
@dp.message_handler(content_types=['new_chat_m
@dp.message_handler(content_types=['new_chembers'])
async def bot_added_to_chat(message: types.Message):
    bot_userat_members'])
async def bot_added_to_chat(message: types.Message):
    bot_user = await bot.me
    if bot_user.id not in = await bot.me
    if bot_user.id [user.id for user in message.new_chat_members]:
        return
    chat = message.chat
    not in [user.id for user in message.new_chat_members]:
        return
    chat = message.chat
    user_id = message.from_user.id
    await create_chat_conf user_id = message.from_user.id
    await create_chat_confirmation_request(chat.id, chat.title, chat.type, user_id)
    for admin_id in SUPER_ADMINS:
        kb = confirm_chat_inirmation_request(chat.id, chat.title, chat.type, user_id)
    for admin_id in SUPER_ADMINS:
        kb = confirm_chline(chat.id)
        await safe_send_message(
            admin_id,
            f"at_inline(chat.id)
        await safe_send_message(
            admin_id,
            f"🆕🆕 Запрос на активацию бота в чате:\n Запрос на активацию бота в чате:\n"
            f"Название: {chat.title}\n"
            f"ID"
            f"Название: {chat.title}\n"
            f"ID: {chat.id}\n"
            f"Тип: {chat.type}\: {chat.id}\n"
            f"Тип: {chat.type}\n"
            f"Запросиn"
            f"Запросил: {message.from_user.first_name} (ID:л: {message.from_user.first_name} (ID {user_id})",
            reply_markup=kb
        )
    await message: {user_id})",
            reply_markup=kb
        )
   .answer("📋 Запрос на активацию бота отправлен await message.answer("📋 Запрос на активацию бота отправлен администратору. Ожидайте подтверждения администратору. Ожидайте подтверждения.")

# ----- ОБРАБОТКА НЕИЗВЕ.")

# ----- ОБРАБОТКА НЕИЗВЕСТНЫХ КОМАНД В ЧАТЕСТНЫХ КОМАНД В ЧАТЕ -----
...
async def unknown_command(message: types.Message):
    ...
    if not await is_chat_confirmed(message.chat.id):
        return
    await message.reply("❌ Неиз await is_chat_confirmed(message.chat.id):
        return
    await message.reply("❌ Неизвестная команда. Введи /help для списка доступных.")

  # ==================== ЧАСТЬ 3: АДМИНИСТРАТИВНЫЕ ХЕНДЛЕРЫ, ФОНОВЫЕ ЗАДАЧИ И MAIN ====================
import asyncio
import logging
import random
...
from core import (
    ...
    ITEMS_PER_PAGE, BIG_WIN_THRESHOLD, BIG_PURCHASE_THRESHOLD,
    MIN_PLAYERS, MAX_PLAYERS, MIN_BET,
    BOSS_SPAWN_PHRASES, BOSS_HIT_PHRASES, BOSS_MISS_PHRASES, BOSS_DEATH_PHRASES,
)
...
# Краткий пример обработчика категории "Кража" (остальные аналогично)
@dp.message_handler(lambda message: message.text == "⚙️ Кража")
async def settings_theft(message: types.Message, state: FSMContext):
    if not await is_super_admin(message.from_user.id):
        return
    # Показываем список параметров кражи с их текущими значениями
    params = [
        "random_attack_cost",
...
async def set_setting_key(message: types.Message, state: FSMContext):
    ...
    # Проверим, существует ли такой ключ в настройках (по умолчанию)
    ...
    while True:
        ...
        try:
            ...
            async with db_pool.acquire() as conn:
                ...
                await conn.execute("DELETE FROM auctions WHERE status='ended' AND end_time < NOW() - INTERVAL '1 day' * $1", days_auctions)
                await conn.execute("DELETE FROM purchases WHERE status IN ('completed','rejected') AND purchase_date < NOW() - INTERVAL '1 day' * $1", days_purchases)
                await conn.execute("DELETE FROM giveaways WHERE status='completed' AND end_date < NOW() - INTERVAL '1 day("DELETE FROM giveaways WHERE status='completed' AND end_date < NOW() - INTERVAL '1 day' * $1", days_give' * $1", days_giveaways)
                await conn.execute("DELETE FROM user_tasks WHERE expires_at IS NOT NULL AND expiresaways)
                await conn.execute("DELETE FROM user_tasks WHERE expires_at IS NOT NULL AND expires_at < NOW()")
                await conn.execute("DELETE FROM fight_logs WHERE timestamp < NOW() - INTERVAL '1 day' * $1", days_at < NOW()")
                await conn.execute("DELETE FROM fight_logs WHERE timestamp < NOW() - INTERVAL '1 day' * $1_fight)
                cooldown = int(await get_setting("fight_cooldown_minutes"))
                await conn.execute", days_fight)
                cooldown = int(await get_setting("fight_cooldown_minutes"))
                await conn.execute("DELETE FROM fight_cooldowns WHERE last_fight < NOW()("DELETE FROM fight_cooldowns WHERE last_fight < NOW() - INTERVAL '1 minute' * $1", cooldown *  - INTERVAL '1 minute' * $1", cooldown * 2)

            logging.info("Автоматическая очистка логов выполнена.")
        except Exception as e:
            logging.error(f"Clean2)

            logging.info("Автоматическая очистка логов выполнена.")
        except Exception as e:
            logging.error(f"Cleanup loopup loop error: {e}")

async def ad_sender_loop():
    while True:
        await asyncio.sleep error: {e}")
...
async def ad_sender_loop():
    while True:
        ...
        if db_pool is None:
            ...
            continue
        try:
            async with db_pool.acquire() as() as conn:
                ...
                for ad in ads:
                    ...
                    if last_sent and ( ad in ads:
                    last_sent = ad['last_sent']
                    if last_sent and (now - last_sent).total_seconds() < ad['interval_minutes'] * 60:
                        continue
now - last_sent).total_seconds() < ad['interval_minutes'] * 60:
                        continue
                    if ad['target'] in ('chats', 'all'):
                        chats = await                    if ad['target'] in ('chats', 'all'):
                        chats = await get_confirmed_chats()
                        chat_ids = list(chats.keys())
                        if chat_ids:
                            chat_id = random.choice(chat_ids)
                            await safe_send_ch get_confirmed_chats()
                        chat_ids = list(chats.keys())
                        if chat_ids:
                            chat_id = random.choice(chat_ids)
                            await safe_sat(chat_id, ad['text'])
                    if ad['target'] in ('private', 'all'):
                        async with db_pool.acquireend_chat(chat_id, ad['text'])
                    if ad['target'] in ('private', 'all'):
                        async with db_pool() as conn2:
                            users = await conn2.fetch("SELECT user_id FROM users WHERE user_id NOT IN (SELECT user_id FROM admins.acquire() as conn2:
                            ...
                            if users:
                                await safe_s) ORDER BY RANDOM() LIMIT 1")
                            if users:
                                await safe_send_message(users[0]['user_id'], ad['text'])
                    await conn.execute("UPDATEend_message(users[0]['user_id'], ad['text'])
                    await conn.execute("UPDATE ads SET last_sent=$1 WHERE id=$2", now, ads SET last_sent=$1 WHERE id=$2", now, ad['id'])
        except Exception as e:
            logging.error(f"Ad sender loop error: {e ad['id'])
        except Exception as e:
...
if __name__ == "__main__":
    while True:
        try:
            ...
            logging.error(f"Критическая_polling(dp, skip_updates=True, on_startup=on_startup, on_shutdown=on_shutdown)
        except TerminatedByOtherGetUpdates:
            logging.error("Конфликт с другим экземпляром. Жду 5 сек...")
            time.sleep(5)
            continue
