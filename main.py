...
async def list_admins(message: types.Message):
    ...
    await message.answer(text)
  # ==================== ЧАСТЬ 8: МУЛЬТИПЛЕЕР, ФОНОВЫЕ ЗАДАЧИ, ВЕБ-СЕРВЕР, ТОЧКА ВХОДА ====================
...
async def multiplayer_menu(message: types.Message):
    ...
    if not ok:
        await message.answer("❗️ Сначала подпишись на каналы.", reply_markup=subscription_inline(not_subscribed))
        return
    await message.answer("🎮 Мультиплеер 21:", reply_markup=games_menu_keyboard())
...
async def list_rooms(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        rooms = await conn.fetch(
            ...
            "(SELECT COUNT(*) FROM game_players WHERE game_id = multiplayer_games.game_id) as players "
    ...
    kb = []
    for room in rooms:
        text += f"🆔 {room['game_id']} | Ставка: {room['bet_amount']} | Игроков: {room['players']}/{room['max_players']}\n"
        kb.append([InlineKeyboardButton(text=f"Войти в {room['game_id']}", callback_data=f"join_room_{room['game_id']}")])
    await message.answer(text, reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
...
async def create_room_max_players(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"Введи ставку (мин. {MIN_BET}, макс. 5):", reply_markup=back_keyboard())
...
async def create_room_bet(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if bet < MIN_BET or bet > 5:
            raise ValueError
    except ValueError:
        await message.answer(f"❌ Ставка должна быть от {MIN_BET} до 5.")
        return
    data = await state.get_data()
    ...
    balance = await get_user_balance(user_id)
    if balance < bet:
        await message.answer("❌ Недостаточно баксов для такой ставки.")
        return
    ...
    async with db_pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO multiplayer_games (game_id, host_id, max_players, bet_amount, deck, created_at, status) VALUES ($1, $2, $3, $4, $5, $6, $7)",
            game_id, user_id, max_players, bet, ','.join(deck), now, 'waiting'
        )
        # Добавляем хозяина как первого игрока
        await conn.execute(
            "INSERT INTO game_players (game_id, user_id, username, cards, joined_at) VALUES ($1, $2, $3, $4, $5)",
            game_id, user_id, message.from_user.first_name, '', now
        )
    await message.answer(
        f"✅ Комната {game_id} создана! Ставка {bet}, макс. {max_players} игроков.\n"
        f"Поделись этим ID с друзьями или жди, пока кто-то присоединится.",
...
async def join_room(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        if balance < room['bet_amount']:
            await callback.answer(f"Недостаточно баксов для ставки {room['bet_amount']}.", show_alert=True)
            return
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        await conn.execute(
            "INSERT INTO game_players (game_id, user_id, username, cards, joined_at) VALUES ($1, $2, $3, $4, $5)",
            game_id, user_id, username, '', now
        )
    await callback.answer(f"✅ Ты присоединился к комнате {game_id}!", show_alert=True)
    await callback.message.delete()
...
async def show_room_status(message: types.Message, game_id: str):
    ...
    text = (
        f"🎮 Комната {game_id}\n"
        f"Ставка: {room['bet_amount']}\n"
        f"Игроки ({len(players)}/{room['max_players']}):\n{players_list}\n\n"
        f"Ожидание начала..."
    )
    if room['host_id'] == message.from_user.id:
        await message.answer(text, reply_markup=room_control_keyboard(game_id))
    else:
        await message.answer(text, reply_markup=leave_room_keyboard(game_id))
...
async def leave_room(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        room = await conn.fetchrow("SELECT * FROM multiplayer_games WHERE game_id=$1 AND status='waiting'", game_id)
        if not room:
            await callback.answer("Комната уже началась или удалена.", show_alert=True)
            return
        await conn.execute("DELETE FROM game_players WHERE game_id=$1 AND user_id=$2", game_id, user_id)
        # Если это был хост и после него остались игроки, назначаем нового хоста
        if room['host_id'] == user_id:
            remaining = await conn.fetch("SELECT user_id FROM game_players WHERE game_id=$1", game_id)
            if remaining:
                new_host = remaining[0]['user_id']
                await conn.execute("UPDATE multiplayer_games SET host_id=$1 WHERE game_id=$2", new_host, game_id)
            else:
                await conn.execute("DELETE FROM multiplayer_games WHERE game_id=$1", game_id)
                await callback.answer("Комната удалена, так как не осталось игроков.", show_alert=True)
                await callback.message.delete()
                return
    await callback.answer("Ты покинул комнату.", show_alert=True)
    await callback.message.delete()
    await list_rooms(callback.message)
...
async def close_room(callback: types.CallbackQuery):
    ...
    user_id = callback.from_user.id
    async with db_pool.acquire() as conn:
        room = await conn.fetchrow("SELECT host_id FROM multiplayer_games WHERE game_id=$1", game_id)
        if not room:
...
async def start_game(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        room = await conn.fetchrow("SELECT * FROM multiplayer_games WHERE game_id=$1 AND status='waiting'", game_id)
        if not room:
            await callback.answer("Комната не найдена или уже начата.", show_alert=True)
            return
        if room['host_id'] != user_id:
        ...
        players = await conn.fetch("SELECT user_id FROM game_players WHERE game_id=$1", game_id)
        if len(players) < 2:
            await callback.answer("Недостаточно игроков (нужно минимум 2).", show_alert=True)
            return
        # Списываем ставки
        for p in players:
            bal = await get_user_balance(p['user_id'])
            if bal < room['bet_amount']:
                await callback.answer(f"Игрок {p['user_id']} не может покрыть ставку. Игра не может начаться.", show_alert=True)
                return
        for p in players:
        ...
        await conn.execute(
            "UPDATE multiplayer_games SET deck=$1, status='playing', current_player_index=0 WHERE game_id=$2",
            ','.join(deck), game_id
        )
        # Отправляем сообщения игрокам
        for p in players:
            player_data = await conn.fetchrow("SELECT * FROM game_players WHERE game_id=$1 AND user_id=$2", game_id, p['user_id'])
        ...
        await notify_next_player(game_id)
    await callback.message.delete()
    await callback.answer()

async def notify_next_player(game_id: str):
    async with db_pool.acquire() as conn:
        room = await conn.fetchrow("SELECT * FROM multiplayer_games WHERE game_id=$1", game_id)
        if not room or room['status'] != 'playing':
            return
        players = await conn.fetch("SELECT user_id FROM game_players WHERE game_id=$1 ORDER BY joined_at", game_id)
        if not players:
            return
        current_idx = room['current_player_index'] % len(players)
        current_user_id = players[current_idx]['user_id']
        await bot.send_message(
            current_user_id,
            "🎯 Твой ход! Выбери действие:",
            reply_markup=room_action_keyboard(game_id, can_double=True)
        )
        # Оповещаем остальных
        for i, p in enumerate(players):
            if i != current_idx:
                await bot.send_message(p['user_id'], f"⏳ Сейчас ходит игрок {current_user_id}.")

@dp.callback_query_handler(lambda c: c.data in ["room_hit", "room_stand", "room_double", "room_surrender", "room_chat"])
async def room_action(callback: types.CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    action = callback.data
    # Найдём игру, где этот пользователь сейчас ходит
    async with db_pool.acquire() as conn:
        room = await conn.fetchrow(
            "SELECT mg.* FROM multiplayer_games mg JOIN game_players gp ON mg.game_id = gp.game_id "
            "WHERE gp.user_id=$1 AND mg.status='playing'", user_id
        )
        if not room:
            await callback.answer("Ты не участвуешь в активной игре или не твой ход.", show_alert=True)
            return
        players = await conn.fetch("SELECT user_id FROM game_players WHERE game_id=$1 ORDER BY joined_at", room['game_id'])
        current_idx = room['current_player_index'] % len(players)
        if players[current_idx]['user_id'] != user_id:
            await callback.answer("Сейчас не твой ход.", show_alert=True)
            return
        player = await conn.fetchrow("SELECT * FROM game_players WHERE game_id=$1 AND user_id=$2", room['game_id'], user_id)
        deck = room['deck'].split(',')
        if action == "room_hit":
            if not deck:
                ...
                return
            new_card = deck.pop()
            cards = player['cards'].split(',') + [new_card]
            value = calculate_hand_value(cards)
            await conn.execute(
                "UPDATE game_players SET cards=$1, value=$2 WHERE game_id=$3 AND user_id=$4",
                ','.join(cards), value, room['game_id'], user_id
            )
            await conn.execute("UPDATE multiplayer_games SET deck=$1 WHERE game_id=$2", ','.join(deck), room['game_id'])
            await callback.message.edit_text(
                f"Ты взял карту {new_card}. Теперь у тебя {value} очков."
            )
            if value > 21:
                # Перебор
                await conn.execute("UPDATE game_players SET stopped=TRUE WHERE game_id=$1 AND user_id=$2", room['game_id'], user_id)
                await callback.message.answer("💥 Перебор! Ты проиграл.")
                await advance_to_next_player(room['game_id'], current_idx, players, conn)
            else:
                # Предложить продолжить
                await callback.message.answer("Твой ход продолжается.", reply_markup=room_action_keyboard(room['game_id'], can_double=False))
        elif action == "room_stand":
            await conn.execute("UPDATE game_players SET stopped=TRUE WHERE game_id=$1 AND user_id=$2", room['game_id'], user_id)
            await callback.message.edit_text("Ты остановился.")
            await advance_to_next_player(room['game_id'], current_idx, players, conn)
        elif action == "room_double":
            # Удвоение ставки – можно только если ещё не удваивал и не брал карты
            if player['doubled'] or player['cards'] != '':
                await callback.answer("Удвоение недоступно.", show_alert=True)
                return
            # Проверим баланс
            bet = room['bet_amount']
            if await get_user_balance(user_id) < bet:
                await callback.answer("Недостаточно баксов для удвоения.", show_alert=True)
                return
            await update_user_balance(user_id, -bet, conn=conn)
            await conn.execute("UPDATE game_players SET doubled=TRUE WHERE game_id=$1 AND user_id=$2", room['game_id'], user_id)
            # Берём одну карту и останавливаемся
            if not deck:
                await callback.answer("Колода пуста!", show_alert=True)
                return
            new_card = deck.pop()
            ...
            await conn.execute("UPDATE multiplayer_games SET deck=$1 WHERE game_id=$2", ','.join(deck), room['game_id'])
            await callback.message.edit_text(f"Ты удвоил и взял карту {new_card}. Итого {value} очков.")
            await advance_to_next_player(room['game_id'], current_idx, players, conn)
        elif action == "room_surrender":
            # Сдаться – возвращаем половину ставки
            bet = room['bet_amount']
            refund = bet // 2
            await update_user_balance(user_id, refund, conn=conn)
            await conn.execute("UPDATE game_players SET surrendered=TRUE, stopped=TRUE WHERE game_id=$1 AND user_id=$2", room['game_id'], user_id)
            await callback.message.edit_text(f"🏳️ Ты сдался. Получил обратно {refund} баксов.")
            await advance_to_next_player(room['game_id'], current_idx, players, conn)
        elif action == "room_chat":
            await callback.message.answer("Введи сообщение для всех игроков комнаты:")
            await state.update_data(room_id=room['game_id'])
            await RoomChat.message.set()
    await callback.answer()

async def advance_to_next_player(game_id: str, current_idx: int, players, conn):
    # Проверим, все ли остановились
    all_stopped = True
    for p in players:
        player = await conn.fetchrow("SELECT stopped FROM game_players WHERE game_id=$1 AND user_id=$2", game_id, p['user_id'])
    ...
    while True:
        player = await conn.fetchrow("SELECT stopped FROM game_players WHERE game_id=$1 AND user_id=$2", game_id, players[next_idx]['user_id'])
        if not player['stopped']:
        ...
        if next_idx == current_idx:
            # Все остановлены (защита от зацикливания)
            await finish_game(game_id)
            return
...
async def room_chat_message room_chat_message(message: types.Message(message: types.Message, state: FSMContext):
    data = await state, state: FSMContext):
    data = await state.get_data.get_data()
 ...
 game_id=$1", game_id)
    for p in players:
        ...
        if p['user_id'] != user_id:
            await safe_send_message(p['user_id'], f"user_id'] != user_id:
            await safe_send_message(p['user_id'],💬 {message.from_user.first_name}: { f"💬 {message.from_user.first_name}: {message.text}")
    await message.answer("✅ Сообщение отправлено всем игрокам.")
message.text}")
    await message.answer("✅ Сообщение отправлено всем игрокам.")
    await state.finish()

@dp.message_handler(lambda    await state.finish()

@dp.message_handler(lambda message: message.text == "🏆 Топ message: message.text == "🏆 Топ мультиплеера")
async def top_multi мультиплеера")
async def top_multiplayer(messageplayer(message: types.Message):
: types.Message):
    if message.ch    if message.chat.type != 'private':
at.type != 'private':
        return
    user_id = message.from_user.id
    if        return
    user_id = message.from_user.id
    if await is await is_banned(user_id) and not await is_admin(user_id):
        return
   _banned(user_id) and not await is_admin(user_id):
        return
 ok, not_subscribed = await check_subscription(user_id)
    if not ok    ok, not_subscribed = await check_subscription(user_id)
:
        await message.answer("❗    if not ok:
        await message.answer("❗️ Сначала подпишись на каналы.", reply_markup=subscription️ Сначала подпишись на каналы.", reply_markup=subscription_inline(not_subscribed))
       _inline(not_subscribed))
        return
    async with db_pool.acquire() as conn:
        rows = await return
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("SELECT first_name, multiplayer_wins FROM users ORDER BY conn.fetch("SELECT first_name, multiplayer_wins FROM users ORDER BY multiplayer_wins DESC LIMIT 10")
    if not rows:
 multiplayer_wins DESC LIMIT 10")
    if not rows:
        await message.answer("        await message.answer("Нет данных.")
        return
    text = "Нет данных.")
        return
    text = "🏆 Т🏆 Топ мультиплеера (по победам):\nоп мультиплеера (по победам):\n\n"
    for i, row in\n"
    for i, row in enumerate(rows, 1):
        text += enumerate(rows, 1):
        text += f"{i}. {row['first_name']} – {row f"{i}. {row['first_name']} – {row['multiplayer_w['multiplayer_wins']} побед\n"
ins']} побед\n"
    await message.answer(text)

@dp.message    await message.answer(text)

@dp.message_handler(lambda_handler(lambda message: message.text == "ℹ message: message.text == "ℹ️ Правила игры")
async def game️ Правила игры")
async def game_rules(message: types.Message):
    await message.answer(
       _rules(message: types.Message):
    await message.answer(
        "📖 Правила игры 21:\n"
        "• Ц "📖 Правила игры 21:\n"
        "ель – набрать очков не больше 21 и больше• Цель – набрать очков не больше 21 и больше, чем у других игроков.\n"
        "• Кар, чем у других игроков.\n"
        "• Карты от 2 до ты от 2 до 10 – по номиналу, J,Q,K – 10, Туз –10 – по номиналу, J,Q,K – 10, Туз – 11 или 1.\n"
        "• Можно брать карты 11 или 1.\n"
        "• Можно брать карты (hit), остановиться (stand), удво (hit), остановиться (stand), удвоить (double) – взять одну карту и остановиться, увеличив ставкуить (double) – взять одну карту и остановиться, увеличив ставку вдвое вдвое.\n"
        "• Сдаться (surrender) – заб.\n"
        "• Сдаться (surrender) – забратьрать половину ставки и выйти из игры.\n"
 половину ставки и выйти из игры.\n"
        "• Если        "• Если у игрока перебор (>21), он проигрывает.\n"
 у игрока перебор (>21), он проигрывает.\n"
        "• Побеждает тот, у кого наиболь        "• Побеждает тот, у кого наибольшая сумма ≤21."
    )

# ====================шая сумма ≤21."
    )

# ==================== ФО ФОНОВЫЕНОВЫЕ ЗАДАЧИ ====================

async ЗАДАЧИ ====================

async def boss def boss_spawn_loop():
    while True_spawn_loop():
    while True:
        await asyncio.sleep(:
        ...
        try:
            confirmed = await get_confirmed_chats()
            await get_confirmed_chats()
            now = datetime.now()
            now = datetime.now()
            for chat_id, data in confirmed.items():
                for chat_id, data in confirmed.items():
                boss_max_per_day = int(await get_setting(" boss_max_per_day = int(await get_setting("boss_max_per_day"))
               boss_max_per_day"))
                boss_spawn_count = data.get('boss_sp boss_spawn_count = data.get('boss_spawn_count', 0)
                if boss_spawn_countawn_count', 0)
                if boss_spawn_count >= boss_max_per_day:
                    continue
                last_spawn_str = data.get(' >= boss_max_per_day:
                    continue
...
boss_last_spawn')
                if last_spawn_str:
                    try:
                        last_spawn = datetime.strptime(last_sp                    try:
                        last_spawn = datetime.strptime(last_spawn_str, "%Y-%awn_str, "%Y-%m-%d %H:%M:%S")
                        minm-%d %H:%M:%S")
                        min_interval = int(await get_setting("_interval = int(await get_setting("boss_min_interval"))
                        if (boss_min_interval"))
                        if (now - last_spnow - last_spawn).total_seconds() < minawn).total_seconds() < min_interval * 60:
                            continue
                   _interval * 60:
                            continue except:
                        pass
                chance = int(await get_setting
                    except:
                        pass
                chance = int(await get_setting("boss_spawn_chance"))
                if random.randint(("boss_spawn_chance"))
                if random.randint(1, 100) <= chance:
                    await spawn_boss(chat_id)
        except Exception1, 100) <= chance:
                    await spawn_boss(chat_id)
        except Exception as e:
 ...
 cleanup_loop():
    while True:
        ...
        try:
            ...
            logging.error(f" as e:
            logging.error(f"Cleanup loop error: {e}")

async def adCleanup loop error: {e}")

async def ad_sender_loop():
   _sender_loop():
    while True:
        await asyncio.sleep( while True:
        ...
        try:
            if db_pool is None:
               _pool is None:
                continue
            async with db continue
            async with db_pool.acquire() as conn:
_pool.acquire() as conn:
                ads = await conn.fetch("SELECT                ads = await conn.fetch("SELECT * FROM ads WHERE enabled=TRUE")
 * FROM ads WHERE enabled=TRUE")
                now = datetime.now()
                for ad in ads:
                now = datetime.now()
                for ad in ads:
                    last_sent = ad                    last_sent = ad['last['last_sent']
                    if last_sent_sent']
                    if last_sent and (now - last_sent).total_seconds() and (now - last_sent).total_seconds() < ad['interval_minutes'] * 60:
                        continue
 < ad['interval_minutes'] * 60:
                        continue
                    if                    if ad['target'] in ('chats', 'all'):
                        chats = await ad['target'] in ('chats', 'all'):
                        chats get_confirmed_chats()
                        chat_ids = = await get_confirmed_chats()
                        chat_ids = list(chats.keys())
                        if chat_ids:
 list(chats.keys())
                        if chat_ids:
                            chat_id = random.choice(chat                            chat_id = random.choice(chat_ids)
                            await safe_s_ids)
                            await safe_send_chat(chat_id, ad['textend_chat(chat_id, ad['text'])
                    if ad['target'] in ('private'])
                    if ad['target'] in ('private', 'all'):
                        users = await conn.fetch("SELECT user_id', 'all'):
                        users = await conn.fetch("SELECT FROM users WHERE user_id NOT IN (SELECT user_id FROM user_id FROM users WHERE user_id NOT IN (SELECT user_id FROM admins) ORDER BY RANDOM admins) ORDER BY RANDOM() LIMIT 1")
() LIMIT 1")
                        if users:
                                                   if users:
                            await safe_send_message(users[0]['user_id'], ad['text'])
 await safe_send_message(users[0]['user_id'], ad['text'])
                    await conn.execute("UPDATE ads SET last_sent=$1 WHERE id=$                    await conn.execute("UPDATE ads SET last_sent=$1 WHERE id=$2", now, ad['id'])
        except2", now, ad['id'])
        except Exception as e:
            logging.error(f"Ad sender loop error: {e}")

async def Exception as e:
            logging.error(f"Ad sender loop error: {e}")

 smuggle_check_loop():
    while True:
        await asyncio.sleepasync def smuggle_check_loop():
    while True:
        await asyncio.sleep(60)
        try:
            async with db_pool.acquire(60)
        try:
            async with db_pool.acquire() as conn:
                runs = await() as conn:
                runs = await conn.fetch(
                    "SELECT * FROM smuggle_runs conn.fetch(
                    "SELECT * FROM smuggle WHERE status='in_progress' AND end_time <= $1 AND_runs WHERE status='in_progress' AND end_time <= $1 AND notified=F notified=FALSE",
                    datetime.now().strftimeALSE",
                    datetime.now().strftime("%Y-%m-%d %H:%M("%Y-%m-%d %H:%M:%S")
               :%S")
                )
                for run in runs:
                    )
 ...
 user_id = run['user_id']
                    ...
                    success_chance = int                    chat_id = run.get('chat_id')
                    success_chance(await get_setting("smuggle_success_chance"))
                    caught = int(await get_setting("smuggle_success_chance"))
                    caught_chance = int(await get_setting("smuggle_caught_chance"))
                   _chance = int(await get_setting("smuggle_caught_chance"))
                    lost_chance = int(await get_setting("smuggle_lost_chance lost_chance = int(await get_setting("smuggle_lost_chance"))
                    ...
                    if rand <= success_chance:
                        result = "success"
                        base = int(await get_set rand <= success_chance:
                        result = "success"
                        base = int(await get_setting("smuggle_base_amount"))
                        auth = await get_user_globalting("smuggle_base_amount"))
                        auth = await get_user_global_authority(user_id)
                        multiplier = 1 + auth * float(_authority(user_id)
                        multiplier = 1 + auth * float(await get_setting("smuggle_authority_multiplier"))
                        amount = int(baseawait get_setting("smuggle_authority_multiplier"))
                        amount = int(base * multiplier)
                        await conn.execute(
                            "UPDATE users SET smuggle_g * multiplier)
                        await conn.execute(
                            "UPDATE users SET smuggle_goods = smuggle_goods + $1, smuggle_successoods = smuggle_goods + $1, smuggle_success = smuggle_success + 1 WHERE user_id=$2",
 = smuggle_success + 1 WHERE user_id=$2",
                            amount                            amount, user_id
                        )
                        notify_text = get_random_phrase(SM, user_id
                        )
                        notify_text = get_random_phrase(SMUGGLE_SUCCESS_PHRUGGLE_SUCCESS_PHRASES, amount=amount)

                    elif rand <= success_chance + caught_chance:
                        result = "ASES, amount=amount)

                    elif rand <= success_chance + caught_chance:
                        result = "caughtcaught"
                        penalty = int(await get_setting("sm"
                        penalty = int(await get_settinguggle_fail_penalty_minutes"))
                        penalty_time =("smuggle_fail_penalty_minutes"))
                        penalty_time = datetime.now() + timedelta(minutes datetime.now() + timedelta(minutes=penalty)
=penalty)
                        await conn.execute('''
                            INSERT INTO global_co                        await conn.execute('''
                            INSERT INTO global_cooldownsoldowns (user_id, command, last_used)
                            VALUES ($1, $ (user_id, command, last_used)
                            VALUES ($1, $2,2, $3)
                            ON CONFLICT (user_id, command) DO UPDATE $3)
                            ON CONFLICT (user_id, command) DO UPDATE SET last SET last_used = $3
                        ''', user_used = $3
                        ''', user_id, "smuggle", penalty_time)
                        await_id, "smuggle", penalty_time)
                        await conn.execute(
                            "UPDATE users SET smuggle_fail = smuggle_f conn.execute(
                            "UPDATE users SET smuggle_fail = smuggle_fail + 1 WHERE user_id=$1",
                            userail + 1 WHERE user_id=$1",
                            user_id
                        )
                        notify_text = get_random_phrase(SMUG_id
                        )
                        notify_text = get_random_phrase(SMUGGGLE_CAUGHT_PHRASES)
                    else:
                        resultLE_CAUGHT_PHRASES)
                    else:
                        result = "lost"
                        await conn.execute(
                            = "lost"
                        await conn.execute(
                            "UPDATE users SET smuggle_fail = smuggle_f "UPDATE users SET smuggle_fail = smuggle_fail + 1 WHERE user_id=$1",
                            userail + 1 WHERE user_id=$1",
                            user_id
                        )
                        notify_text = get_random_phrase(SMUGG_id
                        )
...
LE_LOST_PHRASES)
                    ...
                    await conn.execute(
                        "UPDATE smuggle_runs SET status='completed',                        "UPDATE smuggle_runs SET status='completed', result=$1, notified=TRUE WHERE id=$2",
                        result, run['id']
                    )

                    if chat_id:
                        try result=$1, notified=TRUE WHERE id=$2",
                        result, run['id']
                    )
                    ...
                    if chat_id:
                        try:
                            ...
                            user_name.get_chat_member(chat_id, user_id)).user.first_name
                        except:
                            user_name = f = f"ID {user_id}"
                        await safe_s"ID {user_id}"
                        await safe_send_chat(chat_id, f"end_chat(chat_id, f"🚤 {user_name}, {notify_text}")
🚤 {user_name}, {notify_text}")
                    else:
                        await safe_send_message(user_id, notify_text                    else:
                        await safe_send_message(user_id, notify_text)
        except Exception as e:
            logging.error(f"Smuggle check loop error: {)
        except Exception as e:
            logging.error(f"Smuggle check loop error: {e}")

# ==================== ВЕБe}")

# ==================== ВЕБ-СЕРВ-СЕРВЕР ДЛЯ HEALTH CHECKS =================ЕР ДЛЯ HEALTH CHECKS ====================
...
async def start_web_server():
    app = web.Application()
    app.router.add_get("/", def start_web_server():
    app = web.Application()
    app.router.add_get handle)
    runner = web.AppRunner(app)
("/", handle)
    runner = web.AppRunner(app)
    await    await runner.setup()
    port = int(os.environ.get("PORT", 8080))
    runner.setup()
    port = int(os.environ.get("PORT", 8080))
    site = web.TCPSite(runner, "0.0.0 site = web.TCPSite(runner, "0.0.0.0", port)
    await site.start()
    logging.info(f.0", port)
    await site.start()
    logging.info(f"Web"Web server started on port {port}")

# ==================== ФУНКЦИЯ server started on port {port}")

# ==================== ФУНКЦИЯ ДЛЯ ЗАПУСКА ПЕРЕД СТАРТ ДЛЯ ЗАПУСКА ПЕРЕД СТАОМ ====================
async def before_start():
   РТОМ ====================
async def before_start():
    await bot.delete_webhook(drop_pending_updates=True)
    logging.info("Webhook удалён, проп await bot.delete_webhook(drop_pending_updates=True)
    logging.info("Webhook удалён, пропущены старые обновления")

# ==================== MAIN (ТОЧКАущены старые обновления")

# ==================== MAIN (ТО ВХОДА) ====================

async def on_startup(dp):
    await before_start()
    await createЧКА ВХОДА) ====================

async def on_startup(dp):
    await before_start()
    await create_db_pool()
    await init_db()
    await init_business_db_pool()
    await init_db()
