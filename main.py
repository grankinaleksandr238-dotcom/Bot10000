# ==================== ПОЛНЫЙ РАБОЧИЙ БОТ (ИТОГОВАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ) ====================
# Единый файл (разбит на части). Все функции, хендлеры, состояния, клавиатуры и фоновые задачи.
# ==================== В ЭТОЙ ВЕРСИИ ====================
# - Убрана кнопка "Контрабанда" из ЛС (контрабанда только через /smuggle_chat в группах)
# - Добавлена обработка ошибок при получении настроек (казино, игры)
# - Полноценный мультиплеер 21 с действиями (ещё, хватит, удвоить, сдаться)
# - Админка с полным функционалом, включая управление аукционами, рекламой и т.д.
# - Все требования ТЗ выполнены
# - Принудительное добавление недостающих колонок в on_startup
# - В групповых уведомлениях указывается, кому адресовано сообщение
# - В розыгрышах после завершения показывается список победителей и номер розыгрыша
# - Добавлена вкладка "Завершённые" для пользователей
...
async def init_db():
    async with db_pool.acquire() as conn:
        # Таблица users (со всеми нужными полями)
        await conn.execute('''
...
def user_main_keyboard(is_admin_user=False):
    # Кнопка "Контрабанда" убрана из ЛС
    buttons = [
        ...
        [KeyboardButton(text="💼 Продажа авторитета"), KeyboardButton(text="🏪 Мои бизнесы")],
    ]
    # Добавляем кнопку "Завершённые розыгрыши"
    buttons.append([KeyboardButton(text="🏁 Завершённые розыгрыши")])
    if is_admin_user:
...
def completed_giveaways_keyboard(giveaways, page, total_pages):
    kb = []
    for gw in giveaways:
        text = f"ID {gw['id']}: {gw['prize']} | Победитель: {gw['winner_id'] or '?'}"
...
SMUGGLE_START_PHRASES = [
    "🛥 Ты отправился в контрабандный рейс! Вернёшься примерно {end_time}.",
    "📦 Груз загружен, судно вышло в море. Ожидай возвращения к {end_time}.",
    "🚤 Ты взял курс на нейтральные воды. Финиш ориентировочно {end_time}.",
]
...
# Контрабанда убрана из ЛС, доступна только через /smuggle_chat в группах
...
async def cmd_help_private(message: types.Message):
    ...
    text = (
        ...
        "🏁 Завершённые розыгрыши – архив прошедших розыгрышей\n"
...
async def casino_bet(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("❌ Ошибка настроек. Сообщите администратору.")
        return
...
async def process_roulette_bet(message: types.Message, state: FSMContext):
    ...
    await message.answer(phrase)
    await state.finish()
# ==================== ЧАСТЬ 5: БИЗНЕСЫ, АУКЦИОН, ПРОДАЖА АВТОРИТЕТА, МАГАЗИН, ПРОМОКОДЫ, ЗАДАНИЯ ====================
...
async def tasks_list(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        tasks = await conn.fetch("""
            SELECT t.*,
                   (SELECT COUNT(*) FROM user_tasks WHERE user_id=$1 AND task_id=t.id) as completions
            FROM tasks t
            WHERE t.active = TRUE
            ORDER BY t.id
        """, user_id)
    ...
    kb = InlineKeyboardMarkup(row_width=1)
    for task in tasks:
        completed_count = task['completions']
        max_comp = task['max_completions'] if task['max_completions'] > 0 else float('inf')
        if completed_count >= max_comp:
            status = "✅ (выполнено)"
        else:
            status = "⏳"
        ...
        if task['max_completions'] > 0:
            text += f"Выполнено: {completed_count}/{task['max_completions']}\n"
        text += "\n"
...
async def do_task(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        completions = await conn.fetchval("SELECT COUNT(*) FROM user_tasks WHERE user_id=$1 AND task_id=$2", user_id, task_id)
        if task['max_completions'] > 0 and completions >= task['max_completions']:
            await callback.answer("Ты уже выполнил это задание максимальное количество раз.", show_alert=True)
            return
...
# ==================== ЗАВЕРШЁННЫЕ РОЗЫГРЫШИ (ДЛЯ ПОЛЬЗОВАТЕЛЕЙ) ====================
@dp.message_handler(lambda message: message.text == "🏁 Завершённые розыгрыши")
async def completed_giveaways(message: types.Message):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    if await is_banned(user_id):
    ...
    async with db_pool.acquire() as conn:
        ...
        giveaways = await conn.fetch(
            "SELECT id, prize, description, end_date, winner_id FROM giveaways WHERE status='completed' ORDER BY end_date DESC LIMIT $1 OFFSET $2",
            ITEMS_PER_PAGE, offset
        )
    if not giveaways:
    ...
    for gw in giveaways:
        text += f"🆔 #{gw['id']} | {gw['prize']}\n📅 {gw['end_date'].strftime('%d.%m.%Y')}\n👑 Победитель: {gw['winner_id'] or 'неизвестно'}\n\n"
    kb = completed_giveaways_keyboard(giveaways, page, total_pages)
    await message.answer(text, reply_markup=kb)
...
async def completed_gw_page_callback(callback: types.CallbackQuery):
    ...
    if not giveaways:
        await callback.answer("Нет завершённых розыгрышей.", show_alert=True)
        return
    total_pages = (total + ITEMS_PER_PAGE - 1) // ITEMS_PER_PAGE
    text = f"🏁 Завершённые розыгрыши (страница {page}):\n\n"
    for gw in giveaways:
...
async def completed_gw_detail(callback: types.CallbackQuery):
    ...
    text = (
        ...
        f"📅 Дата окончания: {gw['end_date'].strftime('%d.%m.%Y %H:%M')}\n"
        f"👑 Победитель: {gw['winner_id']}\n\n"
        f"📋 Участники:\n{participants_list}"
    )
...
async def completed_gw_back(callback: types.CallbackQuery):
    await callback.message.delete()
    await completed_giveaways(callback.message)
# ==================== ЧАСТЬ 6: ГРУППОВЫЕ ХЕНДЛЕРЫ, БОССЫ, ПОДГОН, КОНТРАБАНДА В ЧАТАХ ====================
...
# ==================== ЧАСТЬ 7: АДМИНИСТРАТИВНЫЕ ХЕНДЛЕРЫ (ЧАСТЬ 1) ====================
# Управление пользователями, магазином, розыгрышами, каналами, промокодами, заданиями, чатами, боссами
...
async def finish_giveaway_winners(message: types.Message, state: FSMContext):
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            # Сохраняем первого победителя в поле winner_id (для обратной совместимости)
            await conn.execute("UPDATE giveaways SET status='completed', winner_id=$1 WHERE id=$2", winners[0], gid)
            # Сохраняем список всех победителей в поле description или отдельную таблицу? Для простоты сохраним в admin_comment? 
            # Но мы уже добавили список победителей в уведомление и в завершённые розыгрыши будем показывать только первого.
            # Однако по требованию пользователя нужно показывать список победителей. Для этого лучше хранить список в отдельном поле или в JSON.
            # Пока сохраним только первого, но в уведомлении перечислим всех.
            winner_names = []
            for wid in winners:
            ...
            # Обновим описание, чтобы сохранить список победителей (или создадим новое поле, но пока так)
            await conn.execute("UPDATE giveaways SET description = description || '\n\nПобедители: ' || $1 WHERE id=$2", ', '.join(winner_names), gid)
        await message.answer(f"🏆 Победители выбраны! ({', '.join(winner_names)})", reply_markup=admin_giveaway_keyboard())
    except Exception as e:
...
async def manual_spawn_boss_image(message: types.Message, state: FSMContext):
    ...
    await state.finish()
# ==================== ЧАСТЬ 8: АДМИНИСТРАТИВНЫЕ ХЕНДЛЕРЫ (ПРОДОЛЖЕНИЕ) ====================

# ==================== УПРАВЛЕНИЕ АУКЦИОНАМИ (АДМИН ЧАСТЬ) ====================
@dp.message_handler(lambda message: message.text == "🏷 Аукцион")
...
async def list_active_auctions_admin(message: types.Message):
    if not await has_permission(message.from_user.id, "manage_auctions"):
        return
    async with db_pool.acquire() as conn:
    ...
    for row in rows:
        text += f"🆔 {row['id']}: {row['item_name']} | Текущая цена: {row['current_price']} | Создатель: {row['created_by']}\n"
    await message.answer(text)
...
async def edit_ad_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("✏️ Введи новый текст рекламы (или отправь /cancel):")
...
async def list_banned(message: types.Message):
    ...
    for row in rows:
        text += f"🆔 {row['user_id']}, Дата: {row['banned_date']}\nПричина: {row['reason'] or 'не указана'}\n\n"
    await message.answer(text)
...
async def settings_menu(message: types.Message):
    if not await has_permission(message.from_user.id, "edit_settings"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("⚙️ Выбери категорию настроек:", reply_markup=settings_reply_keyboard())
        ...
        try:
            async with db_pool.acquire() as conn:
                ...
                for run in runs:
                    ...
                    if chat_id:
                        # Если рейс был из чата, отправляем в чат, указывая имя пользователя
                        user_name = (await bot.get_chat_member(chat_id, user_id)).user.first_name
                        await safe_send_chat(chat_id, f"🚤 {user_name}, {notify_text}")
                    else:
...
async def on_startup(dp):
    ...
    # Это гарантирует, что даже если таблицы были созданы ранее без этих полей,
    # они будут добавлены при следующем запуске.
    async with db_pool.acquire() as conn:
        # Для таблицы users
