...
async def cmd_start(message: types.Message):
    ...
    try:
        async with db_pool.acquire() as conn:
            # Вставляем пользователя без game_wins (используем только существующие поля)
            await conn.execute(
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if balance < total_price:
                ...
                return
            # Проверяем, что у продавца всё ещё есть авторитет
            ...
            # Списание авторитета у продавца
...
async def admin_helper_menu(message: types.Message):
    if not await has_permission(message.from_user.id, "manage_helpers"):
        await message.answer("❌ Недостаточно прав.")
        return
    # Перенаправляем в меню управления админами
...
async def perform_cleanup(manual=False):
    ...
    async with db_pool.acquire() as conn:
        ...
        # Активные офферы старше 30 дней тоже удаляем (по желанию, можно убрать)
