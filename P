...
# ==================== ИНИЦИАЛИЗАЦИЯ ТАБЛИЦ (СОВМЕСТИМО С СУЩЕСТВУЮЩЕЙ БД) ====================
async def init_db():
    """Создаёт все необходимые таблицы и индексы, если их нет."""
    async with db_pool.acquire() as conn:
        # Таблица users (с текстовыми полями для дат)
        await conn.execute('''
        ...
        # Кулдауны боёв (отдельная таблица)
        await conn.execute('''
...
async def perform_cleanup(manual=False):
    ...
    async with db_pool.acquire() as conn:
        # Для текстовых дат используем преобразование
        cutoff_bosses = (datetime.now() - timedelta(days=days_bosses)).strftime("%Y-%m-%d %H:%M:%S")
        ...
        cutoff_auctions = (datetime.now() - timedelta(days=days_auctions)).strftime("%Y-%m-%d %H:%M:%S")
        await conn.execute("DELETE FROM auctions WHERE status='ended' AND end_time < $1", cutoff_auctions)

        cutoff_purchases = (datetime.now() - timedelta(days=days_purchases)).strftime("%Y-%m-%d %H:%M:%S")
        await conn.execute("DELETE FROM purchases WHERE status IN ('completed','rejected') AND purchase_date < $1", cutoff_purchases)

        cutoff_giveaways = (datetime.now() - timedelta(days=days_giveaways)).strftime("%Y-%m-%d %H:%M:%S")
        await conn.execute("DELETE FROM giveaways WHERE status='completed' AND end_date < $1", cutoff_giveaways)

        await conn.execute("DELETE FROM user_tasks WHERE expires_at IS NOT NULL AND expires_at < $1", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))

        cutoff_fight = (datetime.now() - timedelta(days=days_fight)).strftime("%Y-%m-%d %H:%M:%S")
        await conn.execute("DELETE FROM fight_logs WHERE timestamp < $1", cutoff_fight)

        cooldown = int(await get_setting("fight_cooldown_minutes"))
        cutoff_cooldown = (datetime.now() - timedelta(minutes=cooldown * 2)).strftime("%Y-%m-%d %H:%M:%S")
        ...
        await conn.execute("DELETE FROM authority_offers WHERE status='active' AND created_at < $1", (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%d %H:%M:%S"))

        cutoff_smuggle = (datetime.now() - timedelta(days=days_smuggle)).strftime("%Y-%m-%d %H:%M:%S")
...
# ==================== ФУНКЦИИ ДЛЯ РАБОТЫ СО СТРОКОВЫМИ ДАТАМИ ====================
def str_to_datetime(date_str: str) -> Optional[datetime]:
    """Преобразует строку даты в datetime, если возможно."""
    if not date_str:
        return None
    try:
...
def user_main_keyboard(is_admin_user=False):
    """Главная клавиатура пользователя в ЛС (без кнопки Контрабанда, совместимая со старым кодом)."""
    buttons = [
...
async def check_subscription_callback(callback: types.CallbackQuery):
    ...
    if ok:
        ...
        # Можно обновить клавиатуру, но пока оставим
    ...
    try:
        async with db_pool.acquire() as conn:
            row = await conn.fetchrow(
                ...
                "COALESCE(multiplayer_wins, game_wins, 0) as multiplayer_wins, "
    ...
    async with db_pool.acquire() as conn:
        ...
        if last_bonus_str:
            try:
                ...
                pass  # если дата кривая, игнорируем и выдаём бонус
...
async def show_top(message: types.Message, order_field: str, title: str):
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            # Для мультиплеера используем COALESCE(multiplayer_wins, game_wins, 0)
            if order_field == 'multiplayer_wins':
                order_expr = "COALESCE(multiplayer_wins, game_wins, 0)"
            else:
...
async def process_roulette_bet(message: types.Message, state: FSMContext):
    ...
    await message.answer(phrase)
    await state.finish()
# ==================== ЧАСТЬ 5: МАГАЗИН, ПОКУПКИ, ПРОМОКОДЫ, ОГРАБЛЕНИЕ, РЕФЕРАЛЫ, ЗАДАНИЯ, АУКЦИОН ====================
        ...
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                if success and victim_balance > 0:
                    ...
                    # Проверка на достижение 15 успешных краж для реферала
    ...
    async with db_pool.acquire() as conn:
        ...
        if last_time_str:
            try:
                ...
                pass  # если дата кривая, игнорируем
...
async def cmd_fight(message: types.Message):
    ...
    if outcome == "counter":
        await message.reply(f"💸 Ты потерял {counter_damage} баксов.")
...
async def cmd_boss_status(message: types.Message):
    ...
    if not boss:
        await message.reply("❌ В этом чате нет активного босса.")
        return
...
async def cmd_smuggle_chat(message: types.Message):
    ...
    if not ok:
        ...
        await message.reply(f"⏳ Ты ещё не вернулся из рейса. Подожди {minutes} мин {seconds} сек.")
        return
    ...
    await message.reply(f"🚤 {message.from_user.first_name} отправился в контрабандный рейс! Вернётся примерно {end_time_str}.")
...
async def settings_casino(message: types.Message, state: FSMContext):
    ...
    params = [
        ...
        "guess_multiplier",
        "guess_reputation",
        "slots_multiplier_three",
        "slots_multiplier_diamond",
        "slots_multiplier_seven",
        "slots_min_bet",
        "slots_max_bet",
        "roulette_color_multiplier",
    ...
    for param in params:
        value = await get_setting(param)
        text += f"• {param}: {value}\n"
    text += "\nВведи название параметра для изменения (например, casino_win_chance) или /cancel"
    await message.answer(text)
    await EditSettings.key.set()
...
async def settings_exp(message: types.Message, state: FSMContext):
    ...
    params = [
        ...
        "exp_per_slots_win",
        "exp_per_slots_lose",
        "exp_per_roulette_win",
        "exp_per_roulette_lose",
        "exp_per_theft_success",
        "exp_per_theft_fail",
        "exp_per_theft_defense",
        "exp_per_game_win",
        "exp_per_game_lose",
        "level_multiplier",
        "stat_strength_per_level",
        "stat_agility_per_level",
        "stat_defense_per_level"
    ]
    text = "⚙️ Настройки опыта и уровней:\n"
    for param in params:
        value = await get_setting(param)
        text += f"• {param}: {value}\n"
    text += "\nВведи название параметра для изменения (например, exp_per_casino_win) или /cancel"
    await message.answer(text)
    await EditSettings.key.set()
...
async def settings_boss(message: types.Message, state: FSMContext):
    ...
    params = [
        ...
        "boss_base_damage",
        "boss_reward_coins",
        "boss_reward_coins_variance"
    ]
    text = "⚙️ Настройки боссов:\n"
    for param in params:
        value = await get_setting(param)
        text += f"• {param}: {value}\n"
    text += "\nВведи название параметра для изменения (например, boss_spawn_chance) или /cancel"
    await message.answer(text)
    await EditSettings.key.set()
...
async def settings_auction(message: types.Message, state: FSMContext):
    if not await has_permission(message.from_user.id, "edit_settings"):
        return
    params = [
        "auction_min_bid_step",
        "auction_commission",
        "auction_notify_chats"
    ]
    text = "⚙️ Настройки аукциона:\n"
    for param in params:
        value = await get_setting(param)
        text += f"• {param}: {value}\n"
    text += "\nВведи название параметра для изменения (например, auction_min_bid_step) или /cancel"
    await message.answer(text)
    await EditSettings.key.set()
...
async def settings_gift(message: types.Message, state: FSMContext):
    ...
    for param in params:
        value = await get_setting(param)
        text += f"• {param}: {value}\n"
    text += "\nВведи название параметра для изменения (например, gift_amount) или /cancel"
    await message.answer(text)
    await EditSettings.key.set()
...
async def settings_smuggle(message: types.Message, state: FSMContext):
    if not await has_permission(message.from_user.id, "edit_settings"):
        return
    params = [
        "smuggle_min_duration",
        "smuggle_max_duration",
        "smuggle_success_chance",
        "smuggle_caught_chance",
        "smuggle_lost_chance",
        "smuggle_base_amount",
        "smuggle_authority_multiplier",
        "smuggle_cooldown_minutes",
        "smuggle_fail_penalty_minutes"
    ]
    text = "⚙️ Настройки контрабанды:\n"
    for param in params:
        value = await get_setting(param)
        text += f"• {param}: {value}\n"
    text += "\nВведи название параметра для изменения (например, smuggle_success_chance) или /cancel"
    await message.answer(text)
    await EditSettings.key.set()
...
async def settings_referral(message: types.Message, state: FSMContext):
    ...
    for param in params:
        value = await get_setting(param)
        text += f"• {param}: {value}\n"
    text += "\nВведи название параметра для изменения (например, referral_bonus) или /cancel"
    await message.answer(text)
    await EditSettings.key.set()
...
async def settings_cleanup(message: types.Message, state: FSMContext):
    if not await has_permission(message.from_user.id, "edit_settings"):
        return
    params = [
        "cleanup_days_fight_logs",
        "cleanup_days_bosses",
        "cleanup_days_auctions",
        "cleanup_days_purchases",
        "cleanup_days_giveaways",
        "cleanup_days_user_tasks"
    ]
    text = "⚙️ Настройки очистки логов:\n"
    for param in params:
        value = await get_setting(param)
        text += f"• {param}: {value}\n"
    text += "\nВведи название параметра для изменения (например, cleanup_days_fight_logs) или /cancel"
    await message.answer(text)
    await EditSettings.key.set()
...
async def reject_chat_callback(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        await safe_send_message(request['requested_by'], f"❌ Запрос на активацию чата «{request['title']}» отклонён.")
    await callback.answer()
# ==================== ЧАСТЬ 8: ФОНОВЫЕ ЗАДАЧИ, ВЕБ-СЕРВЕР, ТОЧКА ВХОДА И ЗАПУСК ====================
                ...
                for run in runs:
                    ...
                    if rand <= success_chance:
                        ...
                        await conn.execute(
                            ...
                            (datetime.now() + timedelta(minutes=penalty)).strftime("%Y-%m-%d %H:%M:%S"), user_id
                        )
                        await conn.execute(
...
async def on_startup(dp):
    ...
    async with db_pool.acquire() as conn:
        # Проверяем и добавляем принудительно (для совместимости со старыми БД)
        ...
        await conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS dice_wins INTEGER DEFAULT 0")
        await conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS dice_losses INTEGER DEFAULT 0")
        await conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS slots_wins INTEGER DEFAULT 0")
        await conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS slots_losses INTEGER DEFAULT 0")
        await conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS roulette_wins INTEGER DEFAULT 0")
        await conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS roulette_losses INTEGER DEFAULT 0")
