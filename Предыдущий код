# ==================== ЧАСТЬ 1: ИМПОРТЫ, НАСТРОЙКИ, БАЗА ДАННЫХ, ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ====================
    ...
    async with db_pool.acquire() as conn:
        ...
        if count == 0:
            businesses = [
                ("🗑 Сбор бутылок", 10, 100),
                ("🧹 Мойка окон", 25, 200),
                ("📦 Разгрузка фур", 50, 300),
                ("🍔 Ларёк с шаурмой", 100, 500),
                ("🚕 Такси", 200, 800),
                ("🛠 Маленькая мастерская", 350, 1200),
                ("🍕 Пиццерия", 600, 1800),
                ("🏪 Продуктовый магазин", 1000, 2500),
                ("🏨 Мини-отель", 1500, 3500),
                ("🏭 Небольшой завод", 2500, 5000),
                ("🚛 Логистическая компания", 4000, 7000),
                ("🏦 Частный банк", 6500, 10000),
                ("🛳 Торговый флот", 10000, 15000),
                ("🏢 Небоскрёб", 20000, 22000),
                ("🚀 Космическая программа", 50000, 35000),
            ]
            for name, price, income in businesses:
                await conn.execute(
                    "INSERT INTO business_types (name, emoji, base_price, base_income, description) VALUES ($1, $2, $3, $4, $5)",
                    name, name.split()[0], price, income, f"Бизнес {name}"
                )
...
async def update_user_bitcoin(user_id: int, delta: float, conn=None):
    async def _update(conn):
        ...
        if not row:
            await conn.execute(
                "INSERT INTO users (user_id, bitcoin_balance) VALUES ($1, $2) ON CONFLICT DO NOTHING",
                user_id, 0.0
            )
...
async def get_business_type_list() -> List[dict]:
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("SELECT * FROM business_types ORDER BY base_price")
        return [dict(r) for r in rows]
...
async def get_user_businesses(user_id: int) -> List[dict]:
    async with db_pool.acquire() as conn:
        rows = await conn.fetch("""
            SELECT ub.*, bt.name, bt.emoji, bt.base_price, bt.base_income
            FROM user_businesses ub
            JOIN business_types bt ON ub.business_type_id = bt.id
            WHERE ub.user_id = $1
            ORDER BY bt.base_price
        """, user_id)
        return [dict(r) for r in rows]
...
async def get_business_price(business_type: dict, level: int) -> int:
    if level == 1:
        return business_type['base_price']
    else:
        upgrade_base = int(await get_setting("business_upgrade_cost_per_level"))
        return int(upgrade_base * (level ** 1.5))

async def get_business_income(business_type: dict, level: int) -> int:
    return business_type['base_income'] * level
...
async def update_business_income(user_id: int, conn=None):
    async def _update(conn):
        ...
        businesses = await conn.fetch(
            "SELECT ub.*, bt.base_income FROM user_businesses ub JOIN business_types bt ON ub.business_type_id = bt.id WHERE ub.user_id=$1",
            user_id
        )
        for biz in businesses:
            if biz['last_collection']:
                try:
                    ...
                    if hours_passed > 0:
                        income_per_hour = biz['base_income'] * biz['level']
...
async def upgrade_business(user_id: int, business_id: int) -> Tuple[bool, str]:
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            biz = await conn.fetchrow("SELECT ub.*, bt.base_price, bt.base_income FROM user_businesses ub JOIN business_types bt ON ub.business_type_id = bt.id WHERE ub.id=$1 AND ub.user_id=$2", business_id, user_id)
            if not biz:
                return False, "Бизнес не найден."
            current_level = biz['level']
            if current_level >= 10:
                return False, "Бизнес уже максимального уровня."
            cost = await get_business_price({'base_price': biz['base_price']}, current_level + 1)
            bitcoin_balance = await get_user_bitcoin(user_id)
            if bitcoin_balance < cost:
                return False, f"Недостаточно биткоинов. Нужно {cost} BTC."
            await update_user_bitcoin(user_id, -cost, conn=conn)
            await conn.execute(
                "UPDATE user_businesses SET level = level + 1 WHERE id=$1",
                business_id
            )
            return True, f"✅ Бизнес улучшен до уровня {current_level + 1}! Потрачено {cost} BTC."
...
async def spawn_boss(chat_id: int, level: int = None, image_file_id: str = None):
    ...
    base_reward_coins = int(await get_setting("boss_reward_coins"))
    variance_coins = int(await get_setting("boss_reward_coins_variance"))
    reward_coins = base_reward_coins + random.randint(-variance_coins, variance_coins)
    base_reward_btc = int(await get_setting("boss_reward_bitcoin"))
    variance_btc = int(await get_setting("boss_reward_bitcoin_variance"))
...
async def finish_boss_fight(boss_id: int):
    async with db_pool.acquire() as conn:
        ...
        for i, uid in enumerate(participants):
            ...
            await update_user_balance(uid, coins, conn=conn)
            await update_user_bitcoin(uid, btc, conn=conn)
...
async def perform_cleanup(manual=False):
    ...
    days_orders = int(await get_setting("cleanup_days_bitcoin_orders"))
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if not order:
                return False
            if order['type'] == 'sell':
                await update_user_bitcoin(user_id, order['total_locked'], conn=conn)
            else:
                await update_user_balance(user_id, int(order['total_locked']), conn=conn)
            await conn.execute("UPDATE bitcoin_orders SET status='cancelled' WHERE id=$1", order_id)
            return True

async def execute_bitcoin_trade(buy_order_id: int, sell_order_id: int, amount: float, price: int, buyer_id: int, seller_id: int):
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            buy_order = await conn.fetchrow("SELECT * FROM bitcoin_orders WHERE id=$1 AND type='buy' AND status='active'", buy_order_id)
            sell_order = await conn.fetchrow("SELECT * FROM bitcoin_orders WHERE id=$1 AND type='sell' AND status='active'", sell_order_id)
            if not buy_order or not sell_order:
                raise ValueError("Заявки не найдены или неактивны")
            if buy_order['price'] != price or sell_order['price'] != price:
                raise ValueError("Цены не совпадают")
            if buy_order['amount'] < amount or sell_order['amount'] < amount:
                raise ValueError("Недостаточно средств в заявке")

            total_cost = amount * price

            new_buy_amount = buy_order['amount'] - amount
            new_sell_amount = sell_order['amount'] - amount
            new_buy_locked = buy_order['total_locked'] - total_cost
            new_sell_locked = sell_order['total_locked'] - amount

            if new_buy_amount <= 0:
                await conn.execute("UPDATE bitcoin_orders SET status='completed', amount=0, total_locked=0 WHERE id=$1", buy_order_id)
            else:
                await conn.execute("UPDATE bitcoin_orders SET amount=$1, total_locked=$2 WHERE id=$3", new_buy_amount, new_buy_locked, buy_order_id)

            if new_sell_amount <= 0:
                await conn.execute("UPDATE bitcoin_orders SET status='completed', amount=0, total_locked=0 WHERE id=$1", sell_order_id)
            else:
                await conn.execute("UPDATE bitcoin_orders SET amount=$1, total_locked=$2 WHERE id=$3", new_sell_amount, new_sell_locked, sell_order_id)
            ...
            await conn.execute(
                ...
                buy_order_id, sell_order_id, amount, price, buyer_id, seller_id
            )
...
def bitcoin_orders_keyboard(orders: List[dict], order_type: str, page: int = 1, total_pages: int = 1):
    """Инлайн-клавиатура для отображения списка заявок (покупка/продажа)."""
    kb = []
    for order in orders:
...
def business_buy_keyboard(business_types: List[dict]):
    ...
    for bt in business_types:
        kb.append([InlineKeyboardButton(
            text=f"{bt['emoji']} {bt['name']} – {bt['base_price']} BTC",
...
def admin_main_keyboard(permissions: List[str]):
    ...
    if "manage_exchange" in permissions:
        row.append(KeyboardButton("💼 Биржа"))
    if row:
        buttons.append(row)
    ...
    if "manage_businesses" in permissions:
        row.append(KeyboardButton("🏪 Бизнесы"))
    if "manage_media" in permissions:
        row.append(KeyboardButton("🖼 Медиа"))
    if row:
        buttons.append(row)
...
async def cmd_start(message: types.Message):
    ...
    # Отправляем приветственную картинку если есть
    try:
        async with db_pool.acquire() as conn:
            file_id = await conn.fetchval("SELECT file_id FROM media WHERE key='welcome'")
        if file_id:
            await bot.send_photo(user_id, file_id, caption="Добро пожаловать в Malboro GAME!")
    except:
        pass
...
async def profile_handler(message: types.Message):
    ...
    try:
        async with db_pool.acquire() as conn:
            photo_file_id = await conn.fetchval("SELECT file_id FROM media WHERE key='profile'")
    except:
        pass
    ...
    if photo_file_id:
        await message.answer_photo(photo_file_id, caption=text, reply_markup=main_menu_keyboard(await is_admin(user_id)))
    else:
...
async def reputation_handler(message: types.Message):
    ...
    smuggle_bonus = float(await get_setting("reputation_smuggle_bonus")) * rep
    smuggle_success_bonus = float(await get_setting("reputation_smuggle_success_bonus")) * rep
    await message.answer(
        f"⭐️ Твоя репутация: {rep}\n\n"
        f"Репутация увеличивает шансы и добычу:\n"
...
async def casino_menu(message: types.Message):
    ...
    if not ok:
        await message.answer("❗️ Сначала подпишись на каналы.", reply_markup=subscription_inline(not_subscribed))
        return
    # Проверка общего уровня для доступа к казино
    min_level = int(await get_setting("min_level_casino"))
    level = await get_user_level(user_id)
    if level < min_level:
...
async def casino_bet(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            ...
            btc_reward = int(await get_setting("bitcoin_per_casino_win"))
            if btc_reward > 0:
                await update_user_bitcoin(user_id, btc_reward, conn=conn)
            ...
            if amount * multiplier >= BIG_WIN_THRESHOLD and await get_setting("chat_notify_big_win") == "1":
                await notify_chats(f"🔥 {message.from_user.first_name} сорвал куш в казино: +{amount * multiplier} баксов!{btc_text}")
        else:
...
async def dice_start(message: types.Message):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    min_level = int(await get_setting("min_level_dice"))
    level = await get_user_level(user_id)
    if level < min_level:
        await message.answer(f"❌ Для этой игры нужен {min_level} уровень. Твой уровень: {level}")
        return
...
async def dice_bet(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            ...
            btc_reward = int(await get_setting("bitcoin_per_dice_win"))
            if btc_reward > 0:
                await update_user_bitcoin(user_id, btc_reward, conn=conn)
...
async def guess_start(message: types.Message):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    min_level = int(await get_setting("min_level_guess"))
    level = await get_user_level(user_id)
    if level < min_level:
        await message.answer(f"❌ Для этой игры нужен {min_level} уровень. Твой уровень: {level}")
        return
...
async def guess_number(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            ...
            btc_reward = int(await get_setting("bitcoin_per_guess_win"))
            if btc_reward > 0:
                await update_user_bitcoin(user_id, btc_reward, conn=conn)
...
async def slots_start(message: types.Message):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    min_level = int(await get_setting("min_level_slots"))
    level = await get_user_level(user_id)
    if level < min_level:
        await message.answer(f"❌ Для этой игры нужен {min_level} уровень. Твой уровень: {level}")
        return
...
async def slots_bet(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            ...
            btc_reward = int(await get_setting("bitcoin_per_slots_win"))
            if btc_reward > 0:
                await update_user_bitcoin(user_id, btc_reward, conn=conn)
...
async def roulette_start(message: types.Message):
    if message.chat.type != 'private':
        return
    user_id = message.from_user.id
    min_level = int(await get_setting("min_level_roulette"))
    level = await get_user_level(user_id)
    if level < min_level:
        await message.answer(f"❌ Для этой игры нужен {min_level} уровень. Твой уровень: {level}")
        return
...
async def process_roulette_bet(message: types.Message, state: FSMContext):
    ...
    async with db_pool.acquire() as conn:
        ...
        if win:
            ...
            btc_reward = int(await get_setting("bitcoin_per_roulette_win"))
            if btc_reward > 0:
                await update_user_bitcoin(user_id, btc_reward, conn=conn)
...
async def perform_theft(message: types.Message, robber_id: int, victim_id: int, cost: int = 0):
    ...
    bitcoin_reward = int(await get_setting("bitcoin_per_theft"))
        ...
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                if success and victim_balance > 0:
                    ...
                    if bitcoin_reward > 0:
                        await update_user_bitcoin(robber_id, bitcoin_reward, conn=conn)
...
# ==================== КОНЕЦ ЧАСТИ 3 ====================
# Продолжение (бизнесы, розыгрыши, биткоин-биржа, мультиплеер) будет в Части 4
# ==================== ЧАСТЬ 4: БИЗНЕСЫ, РОЗЫГРЫШИ, БИТКОИН-БИРЖА (ПОЛЬЗОВАТЕЛЬСКИЕ ХЕНДЛЕРЫ) ====================
...
async def my_businesses(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        businesses = await conn.fetch(
            "SELECT ub.*, bt.name, bt.emoji, bt.base_price, bt.base_income FROM user_businesses ub "
            "JOIN business_types bt ON ub.business_type_id = bt.id "
            "WHERE ub.user_id=$1 ORDER BY bt.base_price",
            user_id
        )
...
async def buy_business_menu(callback: types.CallbackQuery):
    """Показывает список доступных для покупки бизнесов."""
    user_id = callback.from_user.id
    # Получаем все типы бизнесов
    all_types = await get_business_type_list()
    # Получаем уже купленные пользователем
    async with db_pool.acquire() as conn:
...
async def buy_business_choose(callback: types.CallbackQuery, state: FSMContext):
    ...
    if existing:
        ...
        return
    price = biz_type['base_price']  # цена за первый уровень
    btc_balance = await get_user_bitcoin(user_id)
    if btc_balance < price:
        await callback.answer(f"Недостаточно биткоинов. Нужно {price} BTC, у тебя {btc_balance:.4f}.", show_alert=True)
        return
    # Подтверждение
    await state.update_data(biz_type_id=biz_type_id, price=price, biz_name=biz_type['name'], biz_emoji=biz_type['emoji'])
    await callback.message.answer(f"Ты уверен, что хочешь купить бизнес «{biz_type['emoji']} {biz_type['name']}» за {price} BTC? (да/нет)", reply_markup=back_keyboard())
...
async def business_view(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        biz = await conn.fetchrow(
            "SELECT ub.*, bt.name, bt.emoji, bt.base_price, bt.base_income FROM user_businesses ub "
            "JOIN business_types bt ON ub.business_type_id = bt.id WHERE ub.id=$1", biz_id
        )
        if not biz:
            await callback.answer("Бизнес не найден", show_alert=True)
            return
    ...
    income_per_hour = biz['base_income'] * biz['level']  # в центах
    income_bucks = income_per_hour // 100
    income_cents = income_per_hour % 100
    upgrade_cost = await get_business_price({'base_price': biz['base_price']}, biz['level'] + 1) if biz['level'] < 10 else 0
    text = (
        f"{biz['emoji']} <b>{biz['name']}</b> (ур. {biz['level']})\n\n"
        f"📈 Доход в час: {income_bucks} баксов {income_cents} центов\n"
        f"💰 Накоплено: {accum_bucks} баксов {accum_cents} центов\n"
    )
    if biz['level'] < 10:
        text += f"⬆️ Стоимость улучшения до ур.{biz['level']+1}: {upgrade_cost} BTC"
    else:
...
async def exchange_order_book(message: types.Message):
    ...
    buy_orders = await get_active_orders('buy')
    sell_orders = await get_active_orders('sell')
    text = "📊 <b>Биржевой стакан</b>\n\n"
    text += "📈 <b>Продажа (ASK)</b>:\n"
    if sell_orders:
        for o in sell_orders[:10]:
            text += f"• {o['amount']:.4f} BTC @ {o['price']} $ (всего {o['amount']*o['price']:.2f} $)\n"
    else:
    ...
    text += "\nИспользуй кнопки ниже для создания заявки или просмотра деталей."
...
async def sell_bitcoin_amount(message: types.Message, state: FSMContext):
    ...
    if btc_balance < amount:
        ...
        return
    min_amount = float(await get_setting("exchange_min_amount_btc"))
...
async def sell_bitcoin_price(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        return
    min_price = int(await get_setting("exchange_min_price"))
    max_price = int(await get_setting("exchange_max_price"))
    if price < min_price:
...
async def buy_bitcoin_start(message: types.Message):
    if message.chat.type != 'private':
        return
    await message.answer(
        "Введи количество BTC, которое хочешь купить (можно дробное, например 0.5):",
        reply_markup=back_keyboard()
    )
...
async def my_orders(message: types.Message):
    ...
    if not rows:
        ...
        return
    # Разбивка по страницам (упрощённо)
    page = 1
...
# ----- Просмотр и исполнение чужих заявок (покупка/продажа) -----
@dp.message_handler(lambda message: message.text == "📈 Купить BTC" and message.chat.type == 'private')
async def buy_from_orders(message: types.Message):
    # Показываем список заявок на продажу
    orders = await get_active_orders('sell')
    if not orders:
        await message.answer("Нет активных заявок на продажу.", reply_markup=bitcoin_exchange_keyboard())
        return
    page = 1
    ...
    kb = bitcoin_orders_keyboard(page_orders, 'sell', page, total_pages)
    await message.answer("📉 <b>Заявки на продажу</b>\nВыбери заявку для покупки:", reply_markup=kb)

@dp.message_handler(lambda message: message.text == "📉 Продать BTC" and message.chat.type == 'private')
async def sell_to_orders(message: types.Message):
    # Показываем список заявок на покупку
    orders = await get_active_orders('buy')
    if not orders:
        await message.answer("Нет активных заявок на покупку.", reply_markup=bitcoin_exchange_keyboard())
        return
    page = 1
    total_pages = (len(orders) + ITEMS_PER_PAGE - 1) // ITEMS_PER_PAGE
    start = (page - 1) * ITEMS_PER_PAGE
    ...
    page_orders = orders[start:end]
    kb = bitcoin_orders_keyboard(page_orders, 'buy', page, total_pages)
    await message.answer("📈 <b>Заявки на покупку</b>\nВыбери заявку для продажи:", reply_markup=kb)

@dp.callback_query_handler(lambda c: c.data.startswith("sell_order_") or c.data.startswith("buy_order_"))
async def execute_order_callback(callback: types.CallbackQuery, state: FSMContext):
    parts = callback.data.split("_")
    order_type = parts[0]  # 'sell' или 'buy'
    ...
    if not order:
        ...
        return
    # Запрашиваем количество для покупки/продажи
    await state.update_data(order_id=order_id, order_type=order_type, max_amount=order['amount'], price=order['price'], seller_id=order['user_id'] if order_type=='sell' else None, buyer_id=order['user_id'] if order_type=='buy' else None)
    await callback.message.answer(
        f"Заявка: {order['amount']:.4f} BTC по цене {order['price']} $/BTC.\n"
        f"Введи количество BTC (не больше {order['amount']:.4f}):",
        reply_markup=back_keyboard()
    )
    # Сохраняем, что мы в процессе исполнения
    await state.set_state("execute_order_amount")
    await callback.answer()

@dp.message_handler(state="execute_order_amount")
async def execute_order_amount(message: types.Message, state: FSMContext):
    if message.text == "◀️ Назад":
        await state.finish()
        await bitcoin_exchange_menu(message)
        return
    ...
    if amount > max_amount:
        await message.answer(f"❌ Нельзя купить больше, чем {max_amount:.4f} BTC.")
        return
    user_id = message.from_user.id

    if order_type == 'sell':
        # Покупатель (текущий пользователь) покупает у продавца
        ...
        buyer_balance = await get_user_balance(user_id)
        total_cost = amount * price
        if buyer_balance < total_cost:
            await message.answer(f"❌ Недостаточно баксов. Нужно {total_cost}.")
            await state.finish()
            return
        try:
            async with db_pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    sell_order = await conn.fetchrow("SELECT * FROM bitcoin_orders WHERE id=$1 AND status='active'", order_id)
                    if not sell_order or sell_order['amount'] < amount - 0.0001:
                        await message.answer("❌ Заявка изменилась. Попробуй снова.")
                        await state.finish()
                        return
                    # Обновляем балансы
                    await update_user_balance(user_id, -total_cost, conn=conn)
                    ...
                    await update_user_balance(seller_id, total_cost, conn=conn)
                    # Уменьшаем заявку
                    new_amount = sell_order['amount'] - amount
                    new_locked = sell_order['total_locked'] - amount
                    ...
                    await conn.execute(
                        ...
                        order_id, amount, price, user_id, seller_id
                    )
            await message.answer(f"✅ Ты купил {amount:.4f} BTC за {total_cost} баксов.", reply_markup=bitcoin_exchange_keyboard())
        except Exception as e:
            logging.error(f"Execute sell order error: {e}")
            await message.answer("❌ Ошибка при исполнении заявки.")
    else:  # order_type == 'buy'
        # Продавец (текущий пользователь) продаёт покупателю
        buyer_id = data['buyer_id']
        if buyer_id == user_id:
            await message.answer("❌ Нельзя продать самому себе.")
            await state.finish()
            return
        # Проверяем баланс продавца (BTC)
        seller_btc = await get_user_bitcoin(user_id)
        if seller_btc < amount:
            await message.answer(f"❌ Недостаточно BTC. У тебя {seller_btc:.4f}.")
            await state.finish()
            return
        try:
            async with db_pool.acquire() as conn:
                async with conn.transaction():
                    buy_order = await conn.fetchrow("SELECT * FROM bitcoin_orders WHERE id=$1 AND status='active'", order_id)
                    if not buy_order or buy_order['amount'] < amount - 0.0001:
                        await message.answer("❌ Заявка изменилась. Попробуй снова.")
                        await state.finish()
                        return
                    total_cost = amount * price
                    ...
                    await update_user_bitcoin(buyer_id, amount, conn=conn)
                    # Уменьшаем заявку
                    new_amount = buy_order['amount'] - amount
                    new_locked = buy_order['total_locked'] - total_cost
                    if new_amount <= 0.0001:
                        await conn.execute("UPDATE bitcoin_orders SET status='completed', amount=0, total_locked=0 WHERE id=$1", order_id)
                    else:
                        await conn.execute("UPDATE bitcoin_orders SET amount=$1, total_locked=$2 WHERE id=$3", new_amount, new_locked, order_id)
                    await conn.execute(
                        ...
                        order_id, amount, price, buyer_id, user_id
                    )
            await message.answer(f"✅ Ты продал {amount:.4f} BTC за {total_cost} баксов.", reply_markup=bitcoin_exchange_keyboard())
        except Exception as e:
            logging.error(f"Execute buy order error: {e}")
            await message.answer("❌ Ошибка при исполнении заявки.")
    await state.finish()

# ==================== КОНЕЦ ЧАСТИ 4 ====================
# ==================== ЧАСТЬ 5: МУЛЬТИПЛЕЕР (ИГРА 21) ====================
...
async def add_player_to_game(game_id: str, user_id: int, username: str):
    async with db_pool.acquire() as conn:
        ...
        players = await conn.fetchval("SELECT COUNT(*) FROM game_players WHERE game_id=$1", game_id)
...
async def get_current_player(game_id: str) -> Optional[dict]:
    async with db_pool.acquire() as conn:
        ...
        if idx >= len(players):
            # Возможно, все закончили – нужно определить победителя
            return None
...
async def next_player(game_id: str) -> Optional[int]:
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if not players:
                return -1
            # Проверяем, все ли закончили (stopped = True)
            ...
            for _ in range(len(players)):
                ...
                if not p['stopped'] and not p['surrendered'] and p['value'] <= 21:
                    ...
                    return next_idx
            # Если все остановились, завершаем
            await finish_game(game_id)
            return -1
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            if not players:
                await conn.execute("DELETE FROM multiplayer_games WHERE game_id=$1", game_id)
                return
            # Определяем победителя: тот, у кого значение <=21 и максимально близко к 21
            best_value = -1
            winner_id = None
            for p in players:
                val = p['value']
            ...
            if winner_id is None:
                # Все перебрали – ничья? возвращаем ставки?
                winner_id = None
            # Выплата: банк = ставка * количество игроков (кроме сдавшихся? но для простоты все)
            ...
            if winner_id:
                ...
                # Ничья – возвращаем ставки? или все проигрывают? оставим как возврат (казино не забирает)
                for p in players:
            ...
            if winner_id:
                await safe_send_message(winner_id, f"🎉 Ты выиграл в игре 21! Твой выигрыш: {pot} баксов.")
...
async def multiplayer_menu(message: types.Message):
    ...
    if not ok:
        ...
        return
    # Проверка уровня
    min_level = int(await get_setting("min_level_multiplayer"))
    level = await get_user_level(user_id)
    if level < min_level:
...
async def create_room_max_players(message: types.Message, state: FSMContext):
    ...
    await message.answer("Введи ставку (целое число, баксы):")
...
async def create_room_bet(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        min_bet = int(await get_setting("multiplayer_min_bet"))
        max_bet = int(await get_setting("multiplayer_max_bet"))
    ...
    # Списываем ставку? пока не списываем, спишем при старте игры
    await state.finish()
    text = (
...
async def list_rooms(message: types.Message):
    ...
    for row in rows:
        ...
        text += f"🆔 {row['game_id']} | Ставка: {row['bet_amount']} | Игроков: {len(players)}/{row['max_players']}\n"
...
async def start_game_callback(callback: types.CallbackQuery):
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                for p in players:
                    balance = await get_user_balance(p['user_id'])
                    if balance < game['bet_amount']:
                        await callback.answer(f"❌ У игрока {p['username']} недостаточно баксов.", show_alert=True)
                        return
                    await update_user_balance(p['user_id'], -game['bet_amount'], conn=conn)
        # Запускаем игру
        await start_game(game_id)
        # Отправляем уведомление всем игрокам о начале
        for p in players:
        ...
        await callback.answer("❌ Ошибка при запуске игры.", show_alert=True)

async def show_current_turn(game_id: str, message: types.Message = None, user_id: int = None):
    """Отправляет сообщение о текущем ходе либо в чат, либо конкретному пользователю."""
    game = await get_multiplayer_game(game_id)
    if not game or game['status'] != 'playing':
        return
    ...
    text += f"\n💰 Твоя ставка: {game['bet_amount']} баксов"
...
async def room_action_callback(callback: types.CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    # Определяем game_id по сообщению? нужно как-то хранить game_id в данных сообщения.
    # Упрощённо: будем искать активную игру для пользователя
    async with db_pool.acquire() as conn:
        # Ищем игру, где пользователь является игроком и статус playing
    ...
    if action == "hit":
        ...
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                # Проверяем, не перебор ли
                if value > 21:
                    # Автоматический проигрыш, ход переходит
        ...
        # Можно удвоить только если ещё не удваивал
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                player = await conn.fetchrow("SELECT * FROM game_players WHERE game_id=$1 AND user_id=$2", game_id, user_id)
                if player['doubled']:
                    ...
                    return
                # Удваиваем ставку: списываем ещё столько же
                game = await conn.fetchrow("SELECT * FROM multiplayer_games WHERE game_id=$1", game_id)
                bet = game['bet_amount']
                balance = await get_user_balance(user_id)
                if balance < bet:
                ...
                # Обновляем ставку в игре? Ставка общая для всех, но для этого игрока увеличится потенциальный выигрыш? Обычно удвоение означает, что игрок берёт одну карту и останавливается.
                # Для простоты: увеличиваем ставку для этого игрока, но выигрыш потом будем считать по общей ставке? Сложно.
                # Упростим: запретим удвоение.
                await callback.answer("❌ Удвоение пока не реализовано.", show_alert=True)
                return
...
async def leave_room_callback(callback: types.CallbackQuery):
    game_id = callback.data.split("_")[2]
    user_id = callback.from_user.id
    # Удаляем игрока из комнаты, если игра ещё не началась
    async with db_pool.acquire() as conn:
        game = await conn.fetchrow("SELECT * FROM multiplayer_games WHERE game_id=$1", game_id)
...
async def cmd_fight(message: types.Message):
    ...
    bitcoin_reward = int(await get_setting("bitcoin_per_fight"))
    ...
    # Начисляем авторитет в чате
    await add_chat_authority(chat_id, user_id, authority, damage)
    # Начисляем биткоины (в глобальный баланс)
    if bitcoin_reward > 0:
        await update_user_bitcoin(user_id, bitcoin_reward)
...
async def cmd_smuggle_chat(message: types.Message):
    ...
    # Устанавливаем кулдаун (начальный, чтобы не запустить второй рейс)
...
async def chat_message_handler(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        # Проверяем кулдаун атаки для этого пользователя
        attack_cooldown = int(await get_setting("boss_attack_cooldown"))
        last_attack = await conn.fetchval(
            "SELECT attack_time FROM boss_attacks WHERE boss_id=$1 AND user_id=$2 ORDER BY attack_time DESC LIMIT 1",
            boss['id'], message.from_user.id
        )
        if last_attack:
            try:
                last = datetime.strptime(last_attack, "%Y-%m-%d %H:%M:%S")
                if (datetime.now() - last).total_seconds() < attack_cooldown:
        ...
        if not await conn.fetchval("SELECT 1 FROM boss_attacks WHERE boss_id=$1 AND user_id=$2", boss['id'], message.from_user.id):
        ...
        await conn.execute(
            "INSERT INTO boss_attacks (boss_id, user_id, damage, attack_time) VALUES ($1, $2, $3, $4) ON CONFLICT (boss_id, user_id) DO UPDATE SET damage = boss_attacks.damage + $3, attack_time = $4",
...
# ==================== КОНЕЦ ЧАСТИ 6 ====================
# ==================== ЧАСТЬ 7: АДМИНИСТРАТИВНАЯ ПАНЕЛЬ (ОСНОВНЫЕ РАЗДЕЛЫ) ====================
...
async def add_balance_start(message: types.Message):
    if not await has_permission(message.from_user.id, "manage_users"):
        await message.answer("❌ Недоститочно прав.")
        return
...
async def finish_giveaway_winners(message: types.Message, state: FSMContext):
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            # Удаляем старые завершённые розыгрыши
            await conn.execute("""
...
# ==================== КОНЕЦ ЧАСТИ 7 ====================
# ==================== ЧАСТЬ 8: АДМИНИСТРАТИВНАЯ ПАНЕЛЬ (ПРОДОЛЖЕНИЕ) ====================
...
async def admin_exchange_menu(message: types.Message):
    if not await has_permission(message.from_user.id, "manage_exchange"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Управление биткоин-биржей:", reply_markup=admin_exchange_keyboard())
...
async def admin_remove_order_finish(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("❌ Введи число.")
        return
    # Пытаемся отменить заявку (независимо от пользователя)
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            order = await conn.fetchrow("SELECT * FROM bitcoin_orders WHERE id=$1 AND status='active'", order_id)
            if not order:
            ...
            # Возвращаем средства
            if order['type'] == 'sell':
                await update_user_bitcoin(order['user_id'], order['total_locked'], conn=conn)
            else:
                await update_user_balance(order['user_id'], int(order['total_locked']), conn=conn)
            await conn.execute("UPDATE bitcoin_orders SET status='cancelled' WHERE id=$1", order_id)
...
async def admin_trade_history(message: types.Message):
    ...
    for r in rows:
        text += f"ID {r['id']}: {r['amount']:.4f} BTC @ {r['price']} $ (покупатель {r['buyer_id']}, продавец {r['seller_id']}) в {r['traded_at']}\n"
    await message.answer(text, reply_markup=admin_exchange_keyboard())
...
async def admin_business_menu(message: types.Message):
    if not await has_permission(message.from_user.id, "manage_businesses"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Управление бизнесами:", reply_markup=admin_business_keyboard())

@dp.message_handler(lambda message: message.text == "📋 Список бизнесов")
async def admin_list_businesses(message: types.Message):
    if not await has_permission(message.from_user.id, "manage_businesses"):
        return
    types = await get_business_type_list()
    if not types:
        await message.answer("Нет типов бизнесов.")
        return
    text = "🏪 Типы бизнесов:\n\n"
    for bt in types:
        text += f"ID {bt['id']}: {bt['emoji']} {bt['name']}\n  Цена: {bt['base_price']} BTC, доход: {bt['base_income']} центов/час\n"
...
async def admin_edit_business_start(message: types.Message):
    if not await has_permission(message.from_user.id, "manage_businesses"):
        return
    await message.answer("Введи ID бизнеса для редактирования:", reply_markup=back_keyboard())
    await EditShopItem.item_id.set()  # можно использовать отдельное состояние, но пока так

# Упрощённо: можно создать отдельные состояния для редактирования бизнеса, но в рамках данного кода оставим заглушку.
@dp.message_handler(state=EditShopItem.item_id)
async def admin_edit_business_not_implemented(message: types.Message, state: FSMContext):
    await message.answer("❌ Редактирование бизнесов пока не реализовано.")
    await state.finish()
    await admin_business_menu(message)

@dp.message_handler(lambda message: message.text == "➕ Добавить бизнес")
async def admin_add_business_start(message: types.Message):
    if not await has_permission(message.from_user.id, "manage_businesses"):
        return
    await message.answer("❌ Добавление бизнесов пока не реализовано.")
    await admin_business_menu(message)

# ==================== УПРАВЛЕНИЕ МЕДИА ====================
@dp.message_handler(lambda message: message.text == "🖼 Медиа")
async def admin_media_menu(message: types.Message):
    if not await has_permission(message.from_user.id, "manage_media"):
        await message.answer("❌ Недостаточно прав.")
        return
    await message.answer("Управление медиафайлами:", reply_markup=admin_media_keyboard())
...
async def stats_handler(message: types.Message):
    ...
    try:
        ...
        text = (
            ...
            f"₿ Всего биткоинов: {total_bitcoin:.4f}\n"
...
async def settings_exp(message: types.Message):
    ...
    params = [
        ...
        ("level_reward_coins", "💰 Базовая награда баксами за уровень"),
        ("level_reward_reputation", "⭐️ Базовая награда репутацией"),
        ("level_reward_coins_increment", "📈 Прирост награды баксами"),
        ("level_reward_reputation_increment", "📈 Прирост награды репутацией"),
...
async def settings_boss(message: types.Message):
    ...
    params = [
        ...
        ("boss_reward_coins", "💰 Базовая награда баксами"),
        ("boss_reward_coins_variance", "🎲 Вариация награды баксами"),
        ("boss_reward_bitcoin", "₿ Базовая награда биткоинами"),
        ("boss_reward_bitcoin_variance", "🎲 Вариация награды биткоинами"),
    ]
...
async def settings_smuggle(message: types.Message):
    ...
    params = [
        ...
        ("smuggle_authority_multiplier", "📈 Множитель от репутации"),
...
@dp.message_handler(lambda message: message.text == "⚙️ Бизнесы")
async def settings_business(message: types.Message):
    if not await has_permission(message.from_user.id, "edit_settings"):
        return
    params = [
        ("business_upgrade_cost_per_level", "⬆️ Базовая стоимость улучшения (BTC)"),
    ]
    await show_settings_category(message, params, "business")

@dp.message_handler(lambda message: message.text == "⚙️ Биткоины")
async def settings_bitcoin(message: types.Message):
    if not await has_permission(message.from_user.id, "edit_settings"):
        return
    params = [
        ("bitcoin_per_theft", "🔫 Биткоины за кражу"),
        ("bitcoin_per_fight", "⚔️ Биткоины за бой"),
        ("bitcoin_per_casino_win", "🎰 Биткоины за победу в казино"),
        ("bitcoin_per_slots_win", "🍒 Биткоины за победу в слотах"),
        ("bitcoin_per_roulette_win", "🎡 Биткоины за победу в рулетке"),
        ("bitcoin_per_dice_win", "🎲 Биткоины за победу в кости"),
        ("bitcoin_per_guess_win", "🔢 Биткоины за победу в угадайке"),
        ("bitcoin_per_boss_participation", "👾 Биткоины за участие в боссе"),
    ]
    await show_settings_category(message, params, "bitcoin")

@dp.message_handler(lambda message: message.text == "⚙️ Биткоин-биржа")
async def settings_exchange(message: types.Message):
    if not await has_permission(message.from_user.id, "edit_settings"):
        return
    params = [
        ("exchange_min_price", "💰 Мин. цена за 1 BTC"),
        ("exchange_max_price", "💰 Макс. цена (0=безлимит)"),
        ("exchange_commission_percent", "💸 Комиссия сделки (%)"),
        ("exchange_commission_side", "👤 С кого брать комиссию (seller/buyer/both)"),
        ("exchange_commission_destination", "🏦 Куда идут комиссионные (burn/balance)"),
        ("exchange_min_amount_btc", "📉 Мин. количество BTC в заявке"),
    ]
    await show_settings_category(message, params, "exchange")
...
async def settings_cleanup(message: types.Message):
    ...
    params = [
        ...
        ("cleanup_days_bitcoin_orders", "💼 Дни хранения заявок"),
    ]
    await show_settings_category(message, params, "cleanup")

@dp.message_handler(lambda message: message.text == "⚙️ Медиа")
async def settings_media(message: types.Message):
    if not await has_permission(message.from_user.id, "edit_settings"):
        return
    params = []  # у медиа нет числовых настроек, можно добавить позже
    await message.answer("⚙️ Настройки медиа: используйте раздел «Медиа» в админке.")
...
async def set_setting_value(message: types.Message, state: FSMContext):
    try:
        # Пытаемся преобразовать в число (можно оставить строкой, но для большинства параметров число)
        value = int(message.text)
    except ValueError:
        # Если не число, оставляем как есть (например, для строковых настроек)
        value = message.text
    data = await state.get_data()
...
async def process_smuggle_runs():
    """
    Проверяет завершённые рейсы контрабанды, вычисляет результат и начисляет награду.
        ...
        try:
            ...
            async with db_pool.acquire() as conn:
                ...
                for run in runs:
                    ...
                    # Получаем репутацию пользователя для расчёта бонусов
                    rep = await get_user_reputation(user_id)
                    ...
                    # Бонус от репутации к шансу успеха
                    rep_success_bonus = float(await get_setting("reputation_smuggle_success_bonus")) * rep
                    total_success_chance = min(success_chance + rep_success_bonus, 100)
                    remaining = 100 - total_success_chance
                    if remaining < 0:
                        remaining = 0
                    # Распределяем оставшийся процент между caught и lost пропорционально их базовым значениям
                    ...
                    if rand <= total_success_chance:
                        ...
                        rep_bonus = float(await get_setting("reputation_smuggle_bonus")) * rep
                    ...
                    await conn.execute(
                        "UPDATE smuggle_runs SET status = $1, notified = TRUE, result = $2 WHERE id = $3",
                        status, result_text, run['id']
                    )

                    # Отправляем результат пользователю в чат (если рейс был начат в чате) или в личку
                    if chat_id:
                        try:
                            # Пытаемся получить имя пользователя для упоминания
                            ...
                            # Если не получилось отправить в чат, шлём в личку
            ...
            async with db_pool.acquire() as conn:
                ...
                for auction in expired:
                    ...
                    if winner_bid:
                        ...
                        await safe_send_message(
                            winner_id,
                            f"🎉 Поздравляем! Вы выиграли аукцион «{auction['item_name']}» с ценой {final_price} баксов. Админ скоро свяжется."
                        )
                        await safe_send_message(
                            auction['created_by'],
                            f"🏁 Аукцион «{auction['item_name']}» завершён. Победитель: {winner_id}, цена: {final_price}."
                        )
                    else:
                        # Нет ставок
                        await conn.execute(
...
async def boss_spawn_scheduler():
    ...
    Запускается каждые 30 минут (1800 секунд).
    """
    while True:
        try:
            ...
            # Пытаемся получить картинку для босса из таблицы media (если есть)
            image_file_id = None
            async with db_pool.acquire() as conn2:
                image_file_id = await conn2.fetchval(
                    "SELECT file_id FROM media WHERE key = 'boss_default'"
                )
        ...
        try:
            ...
            async with db_pool.acquire() as conn:
                ...
                for ad in ads:
                    ...
                    if last_sent:
                        try:
                            # Пытаемся распарсить с микросекундами и без
                            try:
